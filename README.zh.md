...
侧边栏和浏览器标签使用产品插件提供的 Xiaozhuang DSH 品牌名。已有导入聊天与 Agent 会话在相邻历史格式升级后保留标题、分组和最近活动排序；每个分组按 5 → 10 → 20 条逐步展开，方便查找更早的对话。历史转换保留原始文件。Safari 桌面窗口和 Chromium 浏览器通过同一历史读取逻辑展示 Agent 回答及可展开的工具结果。服务重启后，已打开的页面提供手动重新加载入口；登录过期时，没有地址栏的桌面窗口也能在页内恢复登录。各产品设置入口保留原有自定义图标，由各自插件提供并随插件移除。
