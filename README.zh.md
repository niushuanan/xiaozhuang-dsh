...
侧边栏和浏览器标签使用产品插件提供的 Xiaozhuang DSH 品牌名。已有导入聊天与 Agent 会话在相邻历史格式升级后保留标题、分组和最近活动排序；每个分组按 5 → 10 → 20 条逐步展开，方便查找更早的对话。历史转换保留原始文件。 各产品设置入口保留原有自定义图标，由各自插件提供并随插件移除。
