...
| **数字伙伴** | 默认角色“鲸少女”常驻在输入框上方，跟随 Agent 运行、等待、完成和输入框换位；支持蓝黑皮肤、快捷动作与全局自定义名字。 |
...
“鲸少女”是默认名字，“鲸”来自 DeepSeek 的鲸系视觉符号。这个名字不会锁死用户：在“设置 → 数字伙伴”中可以直接修改，保存后角色的辅助名称、右键菜单、显示开关与设置页会立即统一，刷新后仍然保留。插件目录始终使用稳定名称“数字伙伴”，避免用户改名后找不到配置入口。

她常驻在真实输入框右侧正上方，身体可见轮廓刚好贴住上沿，不遮挡输入文字。动作只跟随 Agent 状态和输入框的真实位置，不被鼠标悬停驱动。空闲、专注、等待和完成都使用全趴姿动作；当发送消息或布局变化使输入框换位时，她会以固定尺寸完成两段式传送，到达前不会突然改变大小。空闲呼吸、专注、等待、完成和传送运行帧按 24 fps 曝光表由 `requestAnimationFrame` 对齐显示器刷新，并预解码到单一图片层，减少切帧闪烁。蓝色／黑色皮肤、标准／放大尺寸、任务状态气泡，以及单击、双击、右键的快捷动作都可在独立设置页管理。
...
这个仓库会持续公开已经在日常本地工作中证明有用的插件改进。后续会重点完善 AI 语音输入、整机 Token 总览、更清楚的运行详情、更高效的输入区菜单、更流畅的输出体验，以及更简单的精选插件安装方式。数字伙伴旁的语音图标目前是明确禁用的预告，不会假装已经录音或转写。
