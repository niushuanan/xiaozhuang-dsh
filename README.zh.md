...
<img src="design-qa-product-companion-v2-frames.png" alt="深海蓝与夜航黑小鲸灵在目录、页头、输入区、任务和休息中的连续帧" width="920">

小鲸灵常驻在产品页面之间。重新设计的成年日漫女性角色使用 60 张透明帧，分别覆盖目录、页头、输入区、任务和休息动作；它会显示真实响应阶段和浏览器实际观察到的用时，工作时靠近输入区，完成后短暂庆祝。单击只在原地回应，只有拖动才改变位置；深海蓝／夜航黑换肤和行为开关统一放在独立的“小鲸灵”设置栏目，也可以从“小庄的插件”随时关闭或重新开启。
