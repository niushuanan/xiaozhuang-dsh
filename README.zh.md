...
VSCode 风格右侧栏 + 底部面板：文件与编辑器、真实终端、Git 面板、沙箱内嵌浏览器（替代原浏览器桥接）、后台任务与 Codex 式侧边对话，全部按会话隔离、可拆分合并，并开放 `ctx.betterSidebar` 供后续插件注册页面。它的设置页与产品其余页面共用清晰的标题层级和无外围边框的扁平分区。
