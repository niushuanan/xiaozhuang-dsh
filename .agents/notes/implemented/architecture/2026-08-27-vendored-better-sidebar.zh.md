...
- **浏览器替换。** web-app bundle 移除 Computer Use 的 host/client 挂载行（源码保留可回滚）；vendor 的沙箱 iframe 浏览器、协议分流外链拦截、嵌入拒绝探测与 `browserAllowedLoopback` 白名单成为浏览面。这**部分取代**了[原生 Computer Use 的 Note](../../implemented/feature/2026-08-21-native-computer-use-and-browser-bridge.zh.md)：其中的桥接/隔离浏览器决策随本次变更退役，桌面控制理由与恢复路径继续有效。`sidebar_open` 默认开启，模型可在调用方会话的侧边栏打开文件/文件夹/网页（按会话排队、断线重放）。
...
- 下次启动时 `/computer` 桌面控制与 `/browser`（隔离 Playwright + 已连接桥接）消失；浏览器桥接扩展可以卸载。恢复路径 = 加回两行 bundle 挂载。
