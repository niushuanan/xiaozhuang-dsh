...
The community release version does not change package manifests or claim npm compatibility with a Xiaozhuang version. Future releases update the README asset name, build from the release commit, verify the extracted checkout, and publish a new `xiaozhuang-v*` tag. This checkout's primary `origin` is `niushuanan/xiaozhuang-dsh`; upstream synchronization uses the separate `upstream` remote.
