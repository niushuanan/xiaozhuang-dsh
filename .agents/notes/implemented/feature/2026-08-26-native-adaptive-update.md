...
The detached process performs a real trial merge in a disposable worktree and computes the exact Git conflicts. It does not request a broad semantic review. The process deletes that tree, repeats the pinned merge in a separate candidate, and always invokes one stable Headless Agent on the result. A conflict-free candidate receives a five-minute compatibility pass limited to directly overlapping upstream/plugin files and their contracts; compatible candidates remain unchanged. A conflicting candidate has no artificial timeout, but its prompt is limited to actual conflict files, directly affected plugins, and their direct compile dependencies. Neither path may commit, touch the real DSH Home, run tests, build, install, edit documentation, refactor, or launch subagents.

The private Headless Home receives a one-run Settings overlay that pins `deepseek-official/deepseek-v4-flash-vision-exp`. Conversation model choices never alter this product-owned compatibility route.
...
**Deep semantic review plus exhaustive replay before every update.** This maximizes local evidence but made an ordinary upstream merge take hours. The shipped path still calls Agent for every candidate, but reduces a clean merge to a five-minute direct-contract pass and constrains conflict work to actual conflict files; one production build remains the compatibility boundary, while readiness and rollback remain at cutover.
...
Users can update on demand or opt into six-hour monitoring while continuing to use the old version during compatibility handling and building. Active conversations delay the brief switch, and conversation and attachment storage remains authoritative across the update. Every candidate uses one narrow compatibility request; clean merges are time-bounded, while conflicts are scope-bounded without a forced timeout. One failed build may use one additional narrow repair request. This speed gives up exhaustive behavioral replay before cutover, while the production build, startup readiness check, source rollback, data rollback, bounded temporary checkout, and one retained copy-on-write snapshot preserve the common recovery path.
