...
- 本次任务：处理用户再次看到重新连接表单的反馈，恢复窗口并修复退出桌面 App 后重新打开的真实入口。
- 改了哪些文件：Connection 的 `src/browser-auth.ts`、`src/browser-login-page.ts` 和 `tests/browser-auth.host.spec.ts`，根与 Connection 双语 README、Safari 历史 Agent Note 及各自配对记录，以及本文件。
- 改了什么：未认证的裸根路径 GET 页面只做一次固定同源文档跳转，让桌面窗口带上已保存的 Strict cookie；有效登录清除重试参数，无效登录停在表单，不循环、不签发新 cookie。签名、有效期、HttpOnly、SameSite=Strict 和 API 请求信任检查不变。
- 为什么这样改：退出再打开可重复落入 401，而本机保存的 cookie 未过期且服务端接受；不使用有效令牌、仅同站点表单导航即可恢复，证明首次导航遗漏登录状态。此前刷新和后端重启验收未覆盖桌面 App 冷启动。
- 影响了哪些模块：只影响未认证首页的登录恢复导航，不修改历史、插件、凭据文件、模型或业务执行。第 1–3 节已复核，项目用途、结构与启动入口不变。
- 验证：修复前新增回归失败，修复后 4 个定向文件共 24 项通过；Connection Host 类型检查、定向 lint／构建及 3 组双语结构、链接与 JSDoc 检查通过。330 个会话空闲后按用户既有授权重启服务，进程 1782；实际 Safari 连续两次退出再打开均无需输入令牌，原会话回答和展开的 Bash 结果可读。独立只读安全审查无阻断项。
- 发布范围：仅提交本次改动，保留已有无关工作区记录；主仓推送后仅同步实际受影响的聊天迁移发布副本。未执行哈希值对比。

### 2026-09-05 13:36 - 修复 Safari 桌面 Agent 回答与工具结果缺失

- 本次任务：深入调查实际桌面窗口的大面积 Agent 历史缺失，修复、重启并提交推送；原始会话文件不改写。
- 改了哪些文件：`packages/util/values/src/index.ts`、`packages/core/session/tests/json.spec.ts`，Connection 的浏览器登录页／认证响应／Host 配置清单及测试，frontend-static 的 HTML 缓存／运行实例提示及测试，相关根／包 README、配置目录、双语 Agent Note 和实施计划，以及本文件。
- 改了什么：共享 JSON 校验器改为使用当前引擎的原生构造函数表示，保留名称、原型归属和无损校验；修复 WebKit 多行文本被误判为非法 JSON、导致事件订阅器中断的问题。未认证窗口提供同源令牌表单，首页禁止缓存，实例重启或登录过期时提供手动刷新提示，不自动刷新或修改草稿。
- 为什么这样改：实际 Safari Web App 已连接 WebSocket，却抛出 `Assistant stream raw chunk must be a lossless JSON object`；Chrome 成功不代表桌面版成功。327 对原始／v2 的只读顺序内容核对均完整，共保留 7994 条助手消息、8454 次工具调用、8483 条工具结果，因此修复读取器而非重新导入或改写数据。
- 影响了哪些模块：JSON 容器识别、Agent 历史回放、登录恢复和页面更新提示；未修改 Agent 执行、原生插件目录、模型配置或会话数据。第 1–3 节已复核，项目用途、目录和主入口无需调整。
- 验证：浏览器格式回归在修复前失败，修复后 7 个定向文件共 63 项通过；相关类型检查及源码 lint 通过。实际 Safari 验证用户报告的四轮会话及展开的 Bash 输出、五轮／108 步审查的更早分页和长文件结果、子 Agent 的 35 步完整报告，以及 DeepSeek 导入聊天。临时页面诊断已移除。329 个会话均空闲后重启原 LaunchAgent，进程 10409；实际点击重启提示重连后，历史与导入聊天继续可读。
- 发布范围：只暂存本次文件与本段记录，保留此前 45 行无关项目记录及升级残留；先普通推送主仓库，再从已推送提交同步实际受影响的独立插件副本，不新增 tag 或 Release。禁止哈希对比，文档配对仅记录摘要，验证使用源码、结构及字节内容。
...
- 本次任务：按用户授权继续修复升级后旧 Teamwork 会话无法加载的问题，保持原始历史和可删除文件夹的插件架构。
- 改了哪些文件：`session-format` 通用状态声明，`session-format-v0-to-v1`／`session-format-v1-to-v2` 校验与转换、catalog 回归，JSONL `generation.ts`，`plugins/teamwork/` 的 Host、依赖、测试和说明，`pnpm-lock.yaml`，相关双语包文档和 Agent Note，以及本文件。
- 改了什么：Teamwork 在自身文件夹通过 effect 声明严格的布尔历史状态，核心不认识插件名；v0/v1 迁移保留状态与顺序并在新版标记 ignorable。按已核实旧语义保留三值权限 origin、把精确 descriptor v2 提升 v3，不改权限或模型。发布前用文件身份与原始字节直接比较，代替哈希对比。
- 为什么这样改：未知事件一律放行会误读历史，删除事件会丢失插件状态。由拥有者声明有界语义，再复用不可变新版旁写，可恢复常见旧会话并保留原始证据。
- 影响了哪些模块：只影响历史读取、通用迁移与 Teamwork 声明；不修改插件前端布局、开关功能、价格口径、凭据或原始会话。18 个插件目录仍完整；首次转换未标记历史需安装拥有者，新版文件在插件卸载后仍可读。
- 验证：14 个格式测试文件／266 项、Teamwork 24 项及 JSONL 定向 3 项通过，相关包构建、完整 Host／Client 类型检查与定向 lint 通过。327 份真实旧文件中 325 份通过完整格式复验，并通过标准 read handle 成功发布／重开新版；逐份原始字节、inode、大小与时间均未变。不加载 Teamwork 的独立进程也成功读回新版状态。3080 已按授权重启；原失败短会话可打开，有正文的“简单问候”显示原问候和回复，续聊准确回答原问候“你好”。
- 明确保留：另 2 份更早记录分别是旧 `compact/start` 协议和缺少 `retryId` 的 `llm/retry`，未强行改写；原件与诊断完整保留。尚未转换的无标记插件状态首次迁移时仍需拥有者。第 1～3 节已复核仍准确。

### 2026-09-05 09:13 - Token 分时费用与共享官方价格统一

- 本次任务：补齐 GPT 6、Kimi K3、GLM 5.3 等最新官方定价后，让 Token 总览的分时费用同步使用相同价格，并按用户明确授权重启本机产品。
- 改了哪些文件：`plugins/token-overview/src/hourly-pricing.js`、`plugins/token-overview/src/index.js`、`plugins/token-overview/test/host.test.mjs` 和本文件；此前已更新机器上唯一的 `tokscale-token-report` Skill，其余客户端继续通过原有链接共享。
- 改了什么：把报告 `runtime.pricingRows` 转换为每个报告槽内的独立 Tokscale 价格配置，让原生安全解析器在逐消息计费时应用新价格再汇总分时数据；真实免费模型写入私有目录中的精确零价表。旧分时快照在启动时立即刷新，采集期间继续展示上一份可用报告。
- 为什么这样改：报告总费用已经按官方价格重算，但 native hourly 仍独立使用旧价格，导致同一页面分时费用明显偏低。直接在原生计算入口统一价格可保留 fork 去重和原有小时分组，不需要改 Token 数量、前端布局或插件间依赖。
- 影响了哪些模块：只改变 Token 总览后台分时计费和首次刷新时机；不改全局 Tokscale 配置／缓存、原始会话、历史保护数据或其他插件。第 1–3 节用途、目录和入口已复核，仍准确。
- 验证：9 项 Token 插件定向回归、29 项共享 Skill 官方定价与 DSH 费用回归通过，独立审查无阻断。补齐此前升级遗留的 Host、Client、Web 与 18 个插件产物后，3080 已受控重启；临时移开的未跟踪旧产物目录／文件全部恢复，未删除任一插件。真实设置目录显示 17 项产品能力；Token 报表采集恢复 ready，09:23 快照的分时数据标记为统一价格口径，00–03 的费用由旧价约 $44 修正为约 $175；详细报告确认 GPT 6、Kimi K3、GLM 5.3 的官方单价与来源，控制台 0 error／0 warning。今日总量与小时图为先后扫描，正在发生的调用可造成时间切片差异，不能当作账单。
- 另行发现：启动后打开旧 Teamwork 会话触发上游 v0 历史迁移对 `teamwork/state` 的拒绝，原始日志未变；用户已另行授权继续修复历史兼容。该问题不属于 Token 计费，本条不将其标为已解决。
...
- 本次任务：定位 `0623 新 limix` 工作区“准备开始干活”会话在 Qwen3.8-Max 上显示“本轮运行失败 / 400 status code (no body) / CONTEXT_WINDOW_EXCEEDED”的根因，并修复同类会话续跑。
- 改了哪些文件：`packages/llm/llm-pi-ai/src/context.ts`、`packages/llm/llm-pi-ai/src/stream.ts`、`packages/llm/llm-pi-ai/tests/context.spec.ts`、`packages/llm/llm-pi-ai/tests/convert.spec.ts` 和本文件。
- 改了什么：pi-ai 适配器在回放工具结果前，把除换行、回车、制表符之外的 C0／DEL 控制字符转换为可读的 `\\xNN` 文本，图片请求路径中的工具结果也走同一规则；同时把 pi-ai 仅针对 Cerebras 记录的空 HTTP 400／413 上下文判断限制在 Cerebras，其他自定义 OpenAI 兼容线路按普通无效请求归类。
- 为什么这样改：原始会话压缩后只有约 2.1 万 token，真正触发空 400 的是 `xattr` 工具结果中两个不可打印控制字符。用原会话的 5 条请求消息、63 个工具和 131072 输出上限重放时稳定失败，仅转义这两个字符后 Qwen 网关立即接受；继续把空 400 误标成上下文超限会触发无效压缩并掩盖真实请求校验问题。
- 影响了哪些模块：只影响 `llm-pi-ai` 的工具结果送模表示和终止错误归类；会话日志与界面仍保留原始工具输出，普通用户文本、换行排版、工具执行和其他模型配置不变。项目用途、代码结构和关键入口已复核，第 1–3 节无需调整。
- 验证：两个新增回归用例先分别复现未转义控制字符和错误的上下文归类，修复后 `context.spec.ts` 与 `convert.spec.ts` 共 89 项通过；`llm-pi-ai` TypeScript 构建和四个改动文件的 lint 通过；同一原始请求经修复代码生成约 92 KB 请求体，两个问题字符均成为可见转义且 Qwen3.8-Max 接受。获得用户明确授权后通过 `com.deepseek.harness.web` LaunchAgent 受控重启 3080，服务恢复为单一进程；随后在 `0623 新 limix` 工作区原“准备开始干活”对话中继续沿用同一段长历史，Qwen3.8-Max 正常返回 `DSH_QWEN_CONTROL_CHAR_FIX_OK`，本轮完成且未新增 400 或上下文超限状态。

### 2026-08-30 04:51 - 修复新对话快速切入历史时精灵首次不移动

- 本次任务：修复 Agentic Coding 从居中新对话页切到已有对话时，鲸少女第一次不移动、必须再切一个对话才恢复的问题；要求完成真实入口验收并提交推送。
- 改了哪些文件：`packages/client/ui-product-companion/src/client/ProductCompanion.tsx`、组件定向回归、该包中英文 README／翻译配对记录、新增被打断锚点过渡 Agent Note，以及本文件。
- 改了什么：跨页 360 ms 稳定窗被下一次会话切换打断时，精灵现在会先保留上一页最后实测的输入框泊位作为离场点，再等待新目标稳定并执行既有 `departing → arriving → idle` 材质消散；不会再拿更早页面的旧渲染坐标与新目标比较后错误省略第一次移动。
- 为什么这样改：逐帧真实复现证明问题不是动画素材或第二次点击才生效，而是连续切页取消了上一页待执行定位，导致“居中页实测锚点”和“旧底部渲染坐标”分离；历史页又回到底部后距离变成 0。保留被打断页面的最近可见泊位即可恢复用户所见的正确因果，不需要删除稳定窗、增加状态页或让同页输入框回流反复消散。
- 影响了哪些模块：只影响鲸少女连续跨对话切换时的离场坐标；普通跨页稳定合并、同一对话内输入框高度／附件／面板回流的直接贴随、左右泊位、任务数、拖拽与会话本身均不改变。该改动触及编号 05 映射仓库 `niushuanan/dsh-whale-girl`，主仓推送后需从已推送提交同步；第 1～3 节已复核仍准确。
- 验证：新增回归先在旧实现上稳定收到 `488px` 而非居中泊位 `228px`，证明捕获了产品故障；最小修复后，与普通跨页消散、同页贴随、回流同位不消散四项定向回归全部通过。真实 3080 按“已有对话 → 新对话居中输入框刚出现 → 立即点击已有对话”逐帧复测，第一次点击依次出现 `departing`、`arriving`、`idle`，从居中 `y=230.5` 到底部 `y=524`，最终重新贴合 `composerY=656`，控制台 0 error／0 warning；全程未重启本机 DSH 进程。
...
- 本次任务：把鲸少女下方低价值的麦克风圆键替换成左右泊位切换键；以输入框中位线判断方向，点击后移动到相反一侧约 10%／90% 的位置，并沿用泡沫消散、异地出现的既有移动效果。
- 改了哪些文件：`packages/client/ui-product-companion/src/client/ProductCompanion.tsx`、`ProductCompanion.module.css`、`locales.ts` 与组件测试；该包和根目录的中英文 README／翻译配对记录；既有鲸少女泊位 Agent Note；本文件。
- 改了什么：左圆键现在按鲸少女当前泊位显示左／右箭头；位于中位线右侧时点击写入 `0.1` 泊位，位于左侧时点击写入 `0.9` 泊位。切换复用现有 `departing → arriving → idle` 材质消散链路，人物和快捷键一起在目标泊位重新出现，不做横向滑动。进行中任务数圆键保持不变；语音输入仍可通过 `⌥ Space` 和可配置角色动作使用，但不再占用常驻圆键。
- 为什么这样改：麦克风入口与既有快捷键重复且使用价值低，鲸少女遮挡输入区时却缺少一键避让。一个会自动反向的圆键可以用最少状态完成左右换位；10%／90% 留出边缘呼吸空间，也不会引入额外设置页或三段泊位概念。
- 影响了哪些模块：只影响鲸少女输入框泊位、左侧快捷圆键、对应无障碍文案与语音入口说明；不改变任务统计、拖拽自定义位置、语音能力本身、对话、输入框尺寸或其他插件。该改动触及编号 05 映射仓库 `niushuanan/dsh-whale-girl`，主仓推送后需从已推送提交同步；第 1～3 节已复核仍然准确。
- 验证：新增回归先在旧实现上捕获缺少方向按钮的预期失败；完成后 `ui-product-companion` 全包 3 个测试文件／47 项、TypeScript project build、源码 oxlint、client bundle、双语配对、Agent Note 格式、文档预算与 `git diff --check` 通过。最新源码原位重启 3080 后，真实空白会话从右侧点击到左侧约 10%，采样依次出现 `departing` 与 `arriving`、全程为 `dissolve`，结束后箭头改为向右；再次点击回到右侧约 90% 并恢复向左箭头。任务数按钮仍在、麦克风圆键不再挂载，最终控制台 0 error／0 warning。

### 2026-08-29 16:08 - 设置插件页统一标题与侧边卡片扁平化

- 本次任务：彻底整理设置里的“侧边卡片”页面，移除无意义的插件包标识和大面积框线，统一收起态胶囊开关，并让设置中的所有插件页使用同一标题位置、字号和说明布局。
- 改了哪些文件：在 `packages/client/ui-primitives/src/` 新增共享 `SettingsSectionHeader`；迁移 `ui-adaptive-update`、`ui-agent-preset`、`ui-plugin-catalog`、`ui-product-companion`、`ui-settings-models`、`ui-settings-plugins`、`ui-skill-manager`、`memory-system`、`session-log-export` 和 `better-sidebar` 的设置页及样式；新增共享组件、标题契约和侧边卡片回归测试；补充双语设计／实施文档，并同步根目录 `README.md`、`README.zh.md` 和 `README.i18n.yaml`。为保证本机实际安装页也一致，同步调整 `~/.dsh/profiles/web/packages/{team-work,token-overview}/lib/client.js` 及其契约测试。
- 改了什么：所有插件页改用固定 20px／28px／600 的共享黑字标题、统一说明和操作位；“侧边卡片”删除 `DSH-better-sidebar v0.17.0`，把“常规”“侧边栏内容”“文件预览”改为无外围边框的语义分区，列表只保留必要的行间分隔；关闭态开关改成与产品一致的 36×20 中性灰胶囊、无突兀描边，开启态继续使用黑色胶囊。小庄插件页的导出动作和鲸少女的可编辑名称仍通过共享标题的动作／装饰位保留原功能。
- 为什么这样改：设置页原先由各插件各写一套标题和容器，导致同一层级的页面位置、字号和视觉重量漂移；侧边卡片还暴露包名／版本并用多层圆角框重复分组。把稳定的页面头部下沉为现有基础组件、内容区回归扁平分区，可以用最少概念形成全产品一致的设置体验，同时不改变任何配置项和保存协议。
- 影响了哪些模块：影响设置弹窗中 12 个插件页的页头视觉，以及侧边卡片的分组、列表和开关样式；不改变插件启停、侧边工作台布局、文件预览能力、聊天数据、模型、记忆、导入协议或其他业务状态。本机安装态 Teamwork／Token 总览也使用相同页头，但没有修改其 Host 逻辑和数据请求。
- 验证：受影响包的 Client 测试共 86 个文件／1509 项及本机 Teamwork／Token 总览 13 项契约测试全部通过；12 个相关包的 TypeScript project references 和 bundle 全部通过，`apps/web` Vite 生产构建成功，新增文件与本次改动行定向 lint、根 README 双语配对和 `git diff --check` 通过。全文件 lint 仍会命中 SideCard 旧类型处理、鲸少女旧可选链和插件目录旧 `JSX` 类型共 17 个既有问题，本次不做无关重构。本机 3080 已从当前 checkout 原位重启；真实设置逐页检查“小庄的插件、Skill 管理、导入对话、模型、插件、Agent 预设、Teamwork、Token 总览、长期记忆、鲸少女、持续适配、侧边卡片”共 12 页，标题全部精确位于 x=532、y=154 且为 20px／28px／600。侧边卡片真实页面不再出现包名／版本，三个分区计算样式均为无边框、透明背景、0 圆角，关闭态开关为无边框 36×20 灰色胶囊；浏览器控制台 0 error／0 warning。
...
- 本次任务：修复模型用量面板中刷新图标点下后没有任何视觉反馈、让用户误以为按钮失效的问题；按用户要求不处理本次看到的 GLM 查询状态。
- 改了哪些文件：`packages/client/ui-provider-quota/src/client/QuotaAction.tsx`、`packages/client/ui-provider-quota/tests/quota-action.client.spec.tsx` 和本文件。
- 改了什么：把用量请求的进行中状态从布尔锁改成可等待的请求。当面板打开后正在静默更新时，用户再次手动刷新会立即进入旋转和禁用状态、等待同一个请求完成后恢复，不会重复发送第二次请求；同时在发布旧缓存快照前释放请求引用，保留原有的过期缓存自动更新。
- 为什么这样改：原逻辑在已有静默请求时直接退出，按钮既不旋转也不禁用，造成明确的“点不动”感受；复用正在执行的请求并补齐交互反馈，既让动作即时可感知，也避免重复查询厂商接口。
- 影响了哪些模块：只影响模型用量面板的刷新交互和请求去重；不修改供应商凭据、额度计算、GLM 查询协议、定时刷新周期或其他设置页面。第 1～3 节仍然准确。
- 验证：先用新增回归复现旧实现中按钮保持可用且不旋转，再完成修复；`ui-provider-quota` 全包 3 个测试文件／11 项通过，包级 TypeScript、oxlint 和 bundle 均通过。3080 最新源码真实打开“模型用量”面板，刷新入口可见可用、数据与更新时间正常显示且页面无控制台错误；慢请求下的旋转、禁用、单请求复用和完成后恢复由定向组件回归覆盖。
...
- 本次任务：用户要求不能只验证导入数量，必须在真正迁移的会话中确认历史可见、可以继续对话，并且刷新重开后仍能记住导入前与续聊后的上下文；任何一步失败都要继续修复。
- 改了哪些文件：仓库源码无需修改；通过产品原生模型选择更新本机 `~/.dsh/settings.yaml` 的 `agent-default-model` 为当前真实可用的 Kimi K3，并更新本文件记录真实验收。第 1～3 节的项目用途、结构和入口均未变化。
- 改了什么：先在页面打开迁移历史并确认轮次、原时间、思维过程和正文渲染；续聊时依次捕获当前 DeepSeek 路由密钥无效、GLM 路由余额不足，证明失败来自本机供应商状态而不是导入日志。随后通过现有模型菜单选择 Kimi K3，该选择既写入当前会话的 `model/selection`，也由既有设置服务保存为本机默认，使后续打开的迁移会话不再落到失效路由。验收中特别按 `session-deepseek-*` ID 重新选取真实迁移记录，排除了标题相同的本机旧聊天造成的假阳性。
- 为什么这样改：迁移能力的核心任务是“带着原上下文继续聊”，文件落盘、列表出现或测试通过都不能替代真实模型回合；供应商凭据和额度属于运行环境，现有模型切换链路已能持久解决本机阻断，因此不为环境故障改写导入协议或猜测模型健康状态。
- 影响了哪些模块：只改变本机默认模型与两条用于验收的聊天历史，不改变导入器、Session 事件格式、插件闭包、其他用户的默认配置或 GitHub 发布内容；DeepSeek 导出 ZIP 仍只在本机 Downloads。
- 验证：磁盘上存在 157 份 `session-deepseek-*` 会话日志和 157 份逐会话投影缓存。真实浏览器搜索并打开一条 ID 为 `session-deepseek-*` 的两轮迁移会话，关闭联网后追问一个答案未出现在新提示词、只能由导入前最后一轮得出的事实，Kimi K3 正确回答且无运行失败；完整刷新、切回聊天模式并重新打开同一记录后，旧历史和第一次续聊仍在，再追问上一条答案仍正确。持久投影最终显示 `chat` preset、Kimi K3 已使用、4 轮／4 步、无 open step 或 pending call；浏览器控制台错误为 0。
...
- 本次任务：用户在真实产品中确认侧边／底部面板开合和精灵跨页泡沫切换比改动前更卡，要求先回退这两项优化，恢复可用基线。
- 改了哪些文件：回退 `packages/workbench/better-sidebar/src/client/Sidebar.tsx`、`layout.css`、`sidebar.module.css` 及其新增性能契约测试；回退 `packages/client/ui-product-companion/` 的组件、样式、动画素材索引、Host 静态入口、素材脚本和回归测试，并删除两张仅供该优化使用的长蒙版条；更新本文件。
- 改了什么：精确撤销 `d7140ff0a7` 和 `d14acdd649`。侧边面板恢复改动前的宽度／边距／底部过渡与折叠可见性处理；精灵恢复改动前的 96 帧蒙版 URL 与 React 动画生命周期。之后完成的 DeepSeek 历史导入、联网搜索开关和长历史轮次导航均保留不动。
- 为什么这样改：用户真实体验已经证明两项性能方案没有改善核心交互，反而造成回归；两次提交在相关路径上彼此独立，也没有被后续功能依赖，因此先完整退回已知基线，比继续叠加未经真实验证的优化风险更低。后续如再次优化，应从用户同一路径重新采样和定位。
- 影响了哪些模块：只恢复 better-sidebar 面板开合和 `ui-product-companion` 跨页消散的既有实现，不修改会话、模型、导入数据、Sub-Agent、Teamwork、普通聊天和长历史导航。第 1～3 节已复核仍准确。
- 验证：将两组相关路径分别与各自性能提交的父提交逐文件比较，均无差异；better-sidebar 与 ui-product-companion 合计 42 个测试文件通过，541 项通过、5 项按平台跳过；两个包的 TypeScript project references 和 bundle 均通过。按本机规则未重启 3080，本次结论是已恢复源码基线，不把回退误报为新的性能修复。
...
- 本次任务：用户反馈长对话里打开底部工作台和右侧工作台都会明显卡顿，要求找到共同根因并直接优化。
- 改了哪些文件：`packages/workbench/better-sidebar/src/client/layout.css`、`Sidebar.tsx`、`sidebar.module.css`、新增 `tests/layout-push-styles.client.spec.ts`，以及本文件。
- 改了什么：保留右侧／底部面板自身的 `transform` 滑入动画，但取消主应用 `#root` 的 `width`、`margin-right` 和会话列 `margin-bottom` 的逐帧过渡；面板开合现在只做一次宿主布局提交。折叠面板不再通过 `visibility` 反复让整棵文件树／终端子树失效和重算样式，而是保持在裁剪区外，并用原生 `inert` 与 `aria-hidden` 阻止焦点和无障碍树进入。
- 为什么这样改：真实页面采样确认两个方向的共同瓶颈不是面板位移动画，而是长对话随尺寸动画每帧重排，同时唤醒聊天跟随逻辑、终端尺寸刷新和多个 `ResizeObserver`；右侧面板还会在重新可见时处理数百个文件树节点。一次提交宿主尺寸后，面板位移留在合成层，避免把整页聊天内容拖进每帧热路径。
- 影响了哪些模块：仅影响 better-sidebar 的右侧／底部面板开合和折叠可访问性；面板宽高拖拽、布局占位、文件树、终端、标签页、浮窗、窄屏合并、会话数据和主聊天滚动协议不变。第 1～3 节已复核仍准确。
- 验证：新增性能契约测试先在旧 CSS 上失败；修复后 better-sidebar 全包 40 个测试文件、497 项通过（另 5 项按平台跳过），包级 TypeScript、bundle、变更范围 staged lint 和 `git diff --check` 通过。未重启 3080，直接刷新真实长对话加载新 Client：右侧面板修复前总任务耗时约 764ms，修复后预热三次为 181～220ms、布局约 9～12ms；底部面板由约 228ms 降至 76～119ms、布局约 0.4～1ms。双面板同时打开布局正确，折叠后两者均实际带 `inert=""` 与 `aria-hidden="true"`。

### 2026-08-29 04:05 - 普通聊天联网搜索改为可关闭的状态胶囊

- 本次任务：用户反馈普通聊天输入框里的上传与联网图标间距过小，联网图标灰底突兀且无法关闭；要求在图标／长胶囊两种方向中选择更清晰的方案并一次完成。
- 改了哪些文件：`packages/client/ui-composer-add-menu/` 的组件、样式、双语词典、回归与 README；`packages/client/ui-conversation/` 的输入状态、槽位契约、发送链路与回归；`packages/api/session-controller/` 的 prompt 契约、Client Session、Host 命令处理与回归；以及本文件。
- 改了什么：采用更直观的「联网搜索」长椭圆胶囊，和加号保持 8px 间距；关闭态透明底、灰色边框文字，开启态边框与文字使用产品蓝。胶囊用真实按钮和 `aria-pressed` 表达状态，默认开启、当前页面内可双向切换且不抢输入焦点。发送时把当前开关随 prompt 传到 Host；关闭后 Host 在工具执行入口同时拒绝 `web_search` 与 `web_fetch`，再次开启会卸载门禁，旧 Client 未携带字段时维持原行为。
- 为什么这样改：原实现只是静态说明图标，既没有状态也没有回调，所以用户看到的「联网」无法控制真实能力。把状态放在会话输入属主中，并在 Host 工具执行入口做最终门禁，既保持最短操作路径，又避免只隐藏按钮但模型仍能调用联网工具的假开关。
- 影响了哪些模块：只影响普通聊天的联网控制和对应 prompt 工具权限；Agentic Coding、Sub-Agent、Teamwork、上传菜单、聊天历史和 preset 工具目录不变。第 1～3 节已复核仍准确。
- 验证：菜单／输入壳／发送编排／Host 工具门禁 4 个定向回归文件 116 项通过，完整 Host 构建与 Client 类型检查通过，三个相关包 bundle 和变更文件 lint 通过。真实 3080 页面确认加号 28px、胶囊高 28px、间距 8px；开启态为蓝色，关闭态为透明底灰色，开关连续双向切换且无页面错误。按本机规则未重启 3080，因此真实页面验证覆盖热更新后的 Client 交互；新增 Host 执行门禁由 Host 回归直接验收，下一次正常启动后随最新源码生效。

### 2026-08-29 03:09 - 精灵泡沫消散改由浏览器原生连续蒙版驱动

- 本次任务：用户反馈精灵只在跨页面发生的泡沫消散／重组仍有卡顿，要求找到共同根因并直接优化，同时保持原有消散质感和同页常驻输入框语义。
- 改了哪些文件：`packages/client/ui-product-companion/src/client/ProductCompanion.tsx`、`ProductCompanion.module.css`、`animation.ts`、Host 素材白名单、素材构建脚本、`assets/v13/` 两张新增运行蒙版条、组件／素材回归测试，以及本文件。
- 改了什么：保留原有每阶段 1040ms、48 级人物材质消散和中点换位，但把运行时从 96 次 React 逐帧状态提交、前后帧节点反复挂载和 96 张小蒙版切换，改为两个生命周期稳定的身体／碎片图层；构建阶段将原始 48 张身体和 48 张碎片蒙版分别打成 256px 单元的纵向蒙版条，页面可见时提前解码并持有，切页时由两组 CSS `steps(47)` 动画正反播放。原 96 张高分辨率蒙版和公开 URL 继续保留，避免破坏素材质量检查及兼容调用。
- 为什么这样改：真实页面冷路径捕获到一次切页会请求／解码 96 个蒙版 URL，并在两阶段内做 96 次 React 渲染；即使缓存已热，仍产生约 958 次 DOM 变动、332 次样式重算和 108 次布局。问题来自把视觉帧推进放在 React／DOM 热路径，而不是动画时长本身；缩短动画或减少帧数会牺牲质感，却不能消除共同根因。
- 影响了哪些模块：只影响 `ui-product-companion` 的跨会话消散渲染、素材预解码与 Host 静态分发；同页输入框同步跟随、传送判定、人物原图、48 级节奏、尺寸、拖动、语音、任务控件和会话数据均不改变。第 1～3 节已复核，项目用途、代码结构和关键入口仍准确。
- 验证：素材测试先因两张蒙版条不存在而失败，组件测试先因旧实现仍挂载逐帧 URL 而失败；实现后该包 3 个测试文件／47 项、TypeScript 与 bundle 全部通过。真实 3080 跨页复测中，整页切换期间 DOM 变动由约 958 降至 184、布局由 108 降至 8、脚本耗时由约 170.5ms 降至 66.3ms、总任务耗时由约 519ms 降至 280ms；动画期间始终只存在两个稳定材质图层，采样 169 帧仅 1 次间隔超过 20ms（31.5ms），结束后恢复一个常驻人物图层并准确落到输入框上沿。按本机规则未重启 3080；当前旧 Host 进程不识别新增白名单，真实交互复测使用同一构建产物注入热更新后的客户端，新的 Host 静态路径另由 bundle 后的 handler 直连验收。

### 2026-08-29 02:50 - 长会话轮次导航不再逐页长出和反复重排

- 本次任务：用户反馈进入对话记录特别多的会话时，左侧轮次导航的小间隔条会一根一根出现并伴随明显卡顿，要求直接优化。
- 改了哪些文件：`packages/client/ui-chat/src/client/chat/TurnNavigator.tsx`、`packages/client/ui-chat/src/client/chat/ChatView.tsx`、`packages/client/ui-chat/tests/chat-view.client.spec.tsx` 与本文件。
- 改了什么：轮次导航在有更早历史或分页请求尚未结束时保留首个可用的尾部刻度快照，不再随每个 50 条消息的中间页增加刻度并重算所有位置；历史完整后在同一布局阶段一次发布全量刻度。导航按 `sessionId` 独立重置，并以 `aria-busy` 保留加载语义；首轮／末轮跳转、悬停预览、当前轮高亮和完整历史自动分页均保持原协议。
- 为什么这样改：真实 3080 的 16 轮、1820 个语义行会话会分多页装入；旧实现曾在约 3.6 秒内呈现 `2 → 4 → 5 → … → 16` 的刻度增长，每页都触发已有刻度的 `top` 过渡和新刻度入场动画。用户看到的卡顿来自这组重复视觉提交，而不是十几根线本身。曾验证浏览器 `content-visibility` 离屏裁剪虽然降低主线程时间，却会破坏超长文档滚轮到顶和 End 到底的精确几何，因此已撤回，未拿滚动正确性换跑分。
- 影响了哪些模块：仅影响 `ui-chat` 长历史分页期间的轮次导航发布节奏；不改变会话日志、分页数量、消息 DOM、滚动锚点、折叠内容、搜索、模型或输入框。第 1～3 节已复核仍准确。
- 验证：新增回归先在旧实现上精确捕获中间页新增刻度，修复后 `chat-view.client.spec.tsx` 77／77 通过；88 轮真实浏览器导航测试 2／2 通过，相关 TypeScript、bundle、定向 lint 与 `git diff --check` 通过。冷打开真实 3080 的目标会话时，监测结果由短会话 `0` 根直接收敛为完整 `16` 根，不再暴露中间页刻度；首轮与第 16 轮点击分别正确高亮，外层滚动位置由 `32` 跳到约 `20120.5`。长滚动契约中的三个“滚轮 12 次必须到顶”用例在未带本次生产改动的当前 `HEAD` 上同样失败（仍余约 6.3k px），证明是现有夹具／基线问题；本次未改滚动实现，也未将其误报为通过。
...
- 验证：最终 10 个定向 Vitest 文件 152／152 通过，补充的目录与 Skill 门禁回归 8／8 通过，完整 Host 构建和完整 Client TypeScript 通过；conversation／workspace／composer-add-menu／plugin-catalog 四包 bundle、定向 oxlint、翻译配对和 `git diff --check` 通过，纯聊天闭包实际生成 1.67 MiB 安装 ZIP。全 GUI 回归 4,004 项通过、1 项跳过。全量 Web replay 已完成完整生产构建并进入浏览器阶段，但仓库现有 `settings-chrome.e2e.ts` 出现 9 项失败、`smoke-real.e2e.ts` 出现 6 项失败，其中多数为固定 30 秒超时；该全仓 92 文件长跑在确认失败分布与本次聊天输入路径无关后停止，没有把它当作通过。仓库级 Client 包校验仍被 `ui-conversation` 既有 `session-projection` peer/dev 声明缺口阻断，新增菜单包的 i18n 违规为 0，未越界修改该包声明基线。重启最新 3080 后，真实页面验证聊天加号可读入本地测试文本、地球图标可见；切回 Agentic 精确选中列表首个工作区“迭代DSH”；“文件与文件夹”已启用并列出可进入目录与文件。最小聊天验收实际调用网页搜索并返回 GitHub 链接，最终控制台错误／警告为 0。

### 2026-08-29 00:06 - 恢复 AI 主动记忆与自动整理

- 本次任务：用户发现「设置 → 长期记忆 → AI 主动记忆」一直显示自动整理失败，其中历史会话的 `compact/start` 被新版持久化读取器当成未知事件拒绝；要求恢复真正可用的自动整理，不能只隐藏报错。
- 改了哪些文件：`packages/session/session-persistence/src/coordinator.ts` 及四后端共享契约；`packages/memory/memory-system/src/{maintenance.ts,model.ts,scheduler.ts}` 及对应 maintenance/model/scheduler 回归；本文件。
- 改了什么：持久化读路只对旧版 `compact/start|summary|end|prune` 做精确名称迁移，其他未知事件仍 fail-closed；记忆扫描改读当前会话 surface，只接受真实用户来源和助手可见正文，排除系统提示、插件快照、推理和工具调用参数；单条正文限 1.2 万字符并保留首尾，批次收敛为 6 万字符。后台路由改用纯文本 `deepseek-v4-flash`、关闭推理；大窗口首先按批提取最多 4 千字符的耐久事实（每批 8k token 输出预算），再用一次 32k token 的全局合并重写完整文档。输出改为原生 Markdown 标签边界，保留旧 JSON 兼容，缺少摘要时用稳定默认值。完整文档平时目标为 1.2 万字符、最终硬上限为 6.4 万；首次超过 6.4 万时不落盘，而是立即进入一次专用压缩请求，超过 12.8 万则直接拒绝。多批整理全程在内存中处理、全部成功后只落盘一次；任一后续批失败都不再留下半成品 `ai.md`。
- 为什么这样改：官方合并后的失败是一条连续链路：旧压缩事件名被拒绝 → 通用文本搜索把插件消息和工具参数当成用户记忆 → 巨型 Markdown 被要求 JSON 转义后容易截断或生成非法 JSON → 空摘要又让已合法文档整批失败。修复每个真实阻断点，才能保证游标只在整轮成功后推进。
- 影响了哪些模块：只影响历史会话的兼容读取和 AI 记忆的证据采集／模型维护；不重写、删除或迁移原始会话，不改用户手动记忆。真实验收运行会按既有修订机制写入 `ai.md` 和可回滚历史版本。
- 验证：最新 3080 从游标 `0` 真实完成全历史启动回填，成功写入 28,556 字节的 `ai.md`，游标推进到 `1787933408619` 且错误清空；随后空增量自动维护推进到 `1787933770016`。真实页面「设置 → 长期记忆 → AI主动记忆」可见完整正文且无红色失败提示；点击「立即整理」后按钮正确进入整理中并恢复，页面显示「暂无需要沉淀的新内容」，游标继续推进到 `1787934320116`，正文未被无意义重写，控制台错误／警告为 0。长期记忆 8 个文件 43 项、四种持久化后端 4 个文件 446 项全部通过；两组 TypeScript 构建、定向 oxlint、memory-system client 构建和 `git diff --check` 均通过。第 1–3 节已复核仍准确。

### 2026-08-28 22:20 - 恢复独立聊天分组并移除工作模式中的聊天项

- 本次任务：用户发现顶层「聊天模式」的会话历史被混回普通工作区，而工作会话的 Agent 模式下拉又暴露了内部「聊天」项，要求回到之前独立聊天分组和纯聊天输入的产品形态。
- 改了哪些文件：`packages/client/ui-workspace/` 的会话分组投影、侧栏行、新建聊天入口和定向测试；`packages/client/ui-agent-preset/` 的预设名单、管理页、标题栏标签和测试；`packages/client/ui-conversation/` 的纯聊天输入姿态、文案、槽位契约和测试；以及本文件。
- 改了什么：恢复依据 `projectionValues.agentPreset` 识别纯聊天会话，所有聊天历史统一进入侧栏最前的「聊天」分组，分组悬停加号可复用既有空白聊天或创建新聊天；聊天页不再显示工作区选择、Agent 预设、权限、添加、计划、工作区或分屏控件，只保留模型、上下文和发送。内部 `chat` preset 同时从工作会话的模式下拉、默认模式和预设管理页中隐藏。
- 为什么这样改：官方 `0.1.2-alpha.1` 合并后 SessionSummary 的 Agent preset 从顶层字段迁到 `projectionValues`，原有纯聊天的分组与界面分支在适配中丢失；上一次恢复会话模式下拉时又直接使用完整 roster，进一步把内部组合暴露给用户。本次按用户任务恢复两个互斥的产品入口，不再让用户把「聊天」理解成一种 Agent 工作模式。
- 影响了哪些模块：只影响侧栏会话归类、纯聊天空白页／输入框和 Agent preset 的用户可见性；普通工作会话仍保留标准、PTC、极简、创造四种模式及原有工作控件，不修改历史消息、模型、附件、工作区或用户数据。第 1～3 节已复核，项目用途、结构与关键入口仍然准确，无需调整。
- 验证：两轮回归先分别在旧实现上捕获「`chat` 被当作工作模式」和「纯聊天仍显示工作控件」的失败；修复后 `ui-agent-preset` + `ui-workspace` + `ui-conversation` 11 个定向文件 330 项全部通过，相关源码与测试的 lint、三包 TypeScript 和 client bundle 通过。重启最新 3080 后，真实点击顶层「聊天模式」已显示独立聊天分组、新聊天加号与可输入纯聊天页；切回既有工作会话后，下拉精确包含标准、PTC、极简、创造四项而无聊天，控制台错误／警告为 0。

### 2026-08-28 22:00 - 恢复既有会话的 Agent 模式下拉切换

- 本次任务：用户发现已有对话标题栏中的「标准模式」从可下拉选择退化成静态文字，要求恢复标准、PTC、极简、创造等模式的选择能力。
- 改了哪些文件：`packages/client/ui-agent-preset/` 的标题栏组件、样式、按会话切换控制器、挂载入口、文案、测试及中英文 README／配对记录；`packages/preset/agent-presets/` 的空闲期切换协议、错误语义、测试及中英文 README／配对记录；以及本文件。
- 改了什么：标题栏重新使用现有 Menu 呈现完整 preset 名单和明确下拉箭头；空闲会话立即切换，运行中选择按会话保留到下一空闲边界。Host 用 `Agent.runMaintenance()` 原子重链组装并在提交后记录 `agent-preset/selected`，避免当前轮次执行中途更换工具。客户端在远端提交后立即稳定显示所选模式，同时刷新共享 Session 投影，解决真实页面会永久停在「正在切换」的问题；其他标签页也会随宿主事件收敛。
- 为什么这样改：官方 `0.1.2-alpha.1` 合并覆盖了 8 月 23 日已经完成的实时模式切换：客户端被改回只读 `span`，Host 又恢复成“会话开始后永久锁定”。单纯补箭头只会制造无效控件，必须同时恢复 UI、运行中排队、Host 空闲边界和投影回写这条完整链路。
- 影响了哪些模块：影响既有普通会话标题栏的 Agent preset 展示与后续轮次组装；不会重写历史消息、不会中断当前活跃轮次，也不改变通用设置中的默认模式、新会话模式 chip、模型选择、会话内容或用户数据。第 1～3 节已复核，项目用途、结构和关键入口无需调整。
- 验证：新增回归先在旧实现上得到 4 个预期失败，并在真实页面补获「宿主已提交但标签一直正在切换」的投影缺口；完成后核心 3 个文件 78 项、`ui-agent-preset` 全包 168 项和 `agent-presets` 定向测试全部通过，两个相关包 TypeScript 与客户端 bundle 通过。3080 已重启并在已有 7 轮会话中真实完成「标准 → PTC → 标准」两次切换，菜单含标准、PTC、极简、创造、聊天 5 项，最终无「正在切换」残留，控制台错误／警告为 0。
...
- 改了哪些文件：`packages/client/ui-product-companion/src/client/ProductCompanion.tsx`、`locales.ts`、`tests/product-companion.client.spec.tsx`、该包中英文 README 与翻译配对记录，以及本文件。
- 改了什么：输入框几何观测继续提供真实上沿坐标，但定位分流不再把任意锚点位移当成传送；当前会话未变化时直接采用每次最新坐标，让人物、语音键和任务数键与输入框同帧移动且始终保持常驻。只有 `sessions.current` 变化才开启 360 ms 页面稳定窗，最终目的地与旧页面相差至少 6 px 时播放既有两阶段材质消散；过渡中后续布局变化只更新最终目的地，不会中断动画。回归测试改为明确锁定“同页贴边、跨页消散”两条互斥语义。
- 为什么这样改：旧逻辑把“移动超过 6 px”直接等同于“页面换位”，因此 textarea 自动增高也会在 120 ms 后消散，破坏了精灵趴在输入框上的一体感。动画语义应来自用户是否切换对话页面，而不是来自像素距离；同页布局是父表面自然变化，不是角色旅行。
- 影响了哪些模块：只影响 `ui-product-companion` 的输入框锚点跟随、跨对话传送判定、对应文案与测试；不改变人物素材、横向拖动比例、状态动画、语音、任务列表、输入框本身、会话数据或模型调用。第 1～3 节已复核，项目用途、结构和关键入口无需调整。
- 验证：先观察到新同页回归按预期失败（旧坐标 `488px` 未更新为 `348px`），实现后两个组件／素材测试文件 43／43 通过，包级 TypeScript、插件 bundle、中英文配对与 Web production build 通过。重启最新 3080 后，空白居中页输入框增高／缩短和已有对话输入框 `94→238→94px` 两条真实路径连续采样均始终为 `idle/rest`，人物底边与输入框上沿保持 15px 固定光学校准；从居中空白页切到下方已有对话页则依次出现 `departing → arriving → idle`。最终真实页面控制台错误／警告为 0，验收草稿已清空。
...
- 本次任务：用户反馈轮次进度轨与对话区右缘的工作台控制框重叠，要求把进度轨移到左侧、让消息气泡向右弹出，并让气泡清楚表达“第几轮”和这一轮的用户问题，而不是把轮次名误当作内容。
- 改了哪些文件：`packages/client/ui-chat/src/client/chat/TurnNavigator.tsx`、`TurnNavigator.module.css`、`packages/client/ui-chat/tests/chat-view.client.spec.tsx`、`apps/web/tests/conversation-outline.e2e.ts` 与本文件。
- 改了什么：轮次轨从对话区右缘改为左缘停靠，刻度改为朝内容区左对齐，预览气泡改从轨道右侧展开且入场方向同步；气泡固定以“第 N 轮”为标题，存在用户原话时作为主内容显示，回答摘要作为次级内容，缺少用户原话时不再用轮次名伪装成问题。预览高度同步增加到 120px，保证三层信息仍能完整裁切。
- 为什么这样改：工作台控制框必须贴住对话区右缘，轮次轨也占用同一锚点必然发生竞争；将导航放到左缘可以从布局上消除重叠。原实现又把“第 N 轮”仅作为缺失问题时的回退文本，导致用户看到的气泡无法分清轮次、问题和回答；独立标题与内容层级能直接表达这一轮发生了什么。
- 影响了哪些模块：只影响 `ui-chat` 的轮次导航位置、预览气泡方向和展示层级，以及对应组件／浏览器回归；不改变历史分页、轮次数量、跳转锚点、会话内容、工作台状态或用户数据。第 1～3 节已复核，项目用途、结构和关键入口仍准确。
- 验证：`ui-chat` 25 个测试文件／291 项通过，88 轮 Chromium Web E2E 2/2 通过，Client TypeScript 检查和 `ui-chat` bundle 构建通过。真实 3080 的 7 轮会话在 1200px、右侧 Files 工作台展开和 824px 窄桌面三种状态下均显示左侧轨道、右向气泡与“第 N 轮 + 用户原话”层级；工作台展开时轨道位于 `x=68`、右侧控制框位于 `x=740`，没有重叠，浏览器控制台错误／警告为 0。
...
- 改了哪些文件：`packages/client/ui-outline/` 的 `OutlineRail.tsx`（气泡 Portal 化 + 记录悬停锚点）、`outline.module.css`（卡片改 fixed + 两行截断）、`locales.ts` 不变、测试更新（8 用例）、README/Note 双语同步；`package.json` 声明 `react-dom` 依赖 + `@types/react-dom`；重建产物；本文件。
- 改了什么：① 预览卡改 `createPortal` 到 `body` + fixed 定位（锚在被悬停刻度的视口矩形右侧 12px，垂直中心夹取在视口内），彻底脱离零宽座位与轨道裁剪。② 回答预览改用该轮**最终**一条助手消息的开头文本，`line-clamp` 2 行小字（主人指定“最终回答的前两行”）。③ 问题行有提问行则显示问题，无则回退「第 n 轮」。
- 实机复核：聊天会话 13 轮，悬停第 3 刻度气泡 326×81px 可见（bounding box 验证），内容 = 问题行 + 回答开头两行；截图人工复核通过。
- 影响了哪些模块：只影响 ui-outline 的气泡呈现与预览文本来源；轨道、跳转、滚动侦测不变。刷新页面即生效。第 1–3 节已复核仍准确。

### 2026-08-28 01:30 - 大纲轨改按轮次边界推导 + 有界自动扩窗（修复不显示）

- 本次任务：主人重启后反馈大纲轨仍然看不到。实机排查发现两个根因：① 编码视图不把用户提问渲染成独立消息行（DOM 探测 78 个顶层行中 0 个 user 节点），v1 把“轮”锚在 user 节点上导致轮数算出 0；② 聊天记录是分页窗口，工具量巨大的会话首页只有 1-2 轮，阈值 3 永远达不到。
- 改了哪些文件：`packages/client/ui-outline/` 的 `OutlineRail.tsx`（推导重写 + 扩窗）、`client/index.ts`（注入 `loadOlder` 回调，inject 增加 `sessions`）、`locales.ts`（`outline.turn.unnamed`）、测试重写（8 用例）、README 双语与 Note 三件套就地更新；重建 ui-outline client 产物；本文件。
- 改了什么：① 轮次边界改从每条可见行的 `location.turn` 推导，不再依赖 user 节点；`user`/`steering` 行只把截断问题文本贡献给下一轮预览（没有提问行时回退「第 n 轮」文案，预览显示回答开头）。② 注入面携带会话对象面 `loadOlder`：窗口不足三轮且还有更早历史时自动回拉，每次挂载至多 3 页。③ 有界、只读、无模型可见物。
- 实机复核：编码会话（深度分析）3 刻度 + 悬停卡 + 点击跳转（scrollTop 17128→4639）；聊天会话（疲惫一天）13 刻度；1-2 轮的短会话正确隐藏。类型/lint/8 测试全绿。
- 影响了哪些模块：只影响 ui-outline 插件自身；座位、槽位契约与其他插件不变。master 刷新页面即可生效（bundle 按请求下发，无需重启）。第 1–3 节已复核仍准确。
...
- 改了哪些文件：`packages/client/ui-conversation/src/client/skeleton/InputBar.tsx` 与 `tests/input-bar.client.spec.tsx`（原停止态用例改写为草稿态用例 + 新增空输入框停止态用例），重建 ui-conversation client 产物；本文件。
- 改了什么：主按钮状态条件从 `running && subagent === null` 收紧为 `running && subagent === null && empty`——有文字或图片草稿时主按钮回到「发送」，点击走既有 submit('queue') 入队路径；输入框为空时运行中仍是「停止」。仅普通会话分支；continuable 子会话的 Send+独立 Stop 不变。
