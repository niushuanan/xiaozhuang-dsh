...
- 验证：两个新增回归用例先分别复现未转义控制字符和错误的上下文归类，修复后 `context.spec.ts` 与 `convert.spec.ts` 共 89 项通过；`llm-pi-ai` TypeScript 构建和四个改动文件的 lint 通过；同一原始请求经修复代码生成约 92 KB 请求体，两个问题字符均成为可见转义且 Qwen3.8-Max 接受。当前 3080 是 9 月 2 日启动、Host 模块已缓存的进程，真实页面第三轮仍按旧代码返回空 400；按本机进程保护规则尚未擅自重启，需获用户允许后重启并完成最终页面复验。
