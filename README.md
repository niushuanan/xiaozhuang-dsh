...
| **Native plain chat** | Start a durable conversation without choosing a folder or granting Agent capabilities; the familiar chat page keeps model choice, history, and compact image/text-file uploads while removing Workspace, mode, Tool, and permission controls. |
...
| **Commands, plugins and skills** | In work conversations, the composer plus button opens one native directory for images, Workspace references, the live official command catalog, plugins, and Skills; a pick inserts the slash command without executing it. |
...
| **Long-term memory** | Edit separate user and AI memory documents; AI maintains its document daily and only recalls small relevant pieces for later work. |
...
| **Searchable plugin catalog** | “Xiaozhuang's plugins” groups capabilities into Work, Conversation, and Data & Usage sections, searches names, descriptions, metadata, and categories, mirrors each capability's current product name and glyph, and shows this MIT repository as an explicit blue, clickable Star link. |
...
The sidebar separates intent before a conversation starts: **Start work** follows the current Workspace flow, while **Start chat** opens or reuses a blank conversation under a dedicated **Chats** group. If the user changes their mind while either start is still resolving, the latest click wins instead of an older asynchronous result taking the page back. Chat uses an internal no-Tool preset, is not attached to a folder, and omits Workspace, mode, Skill, command, and permission controls. Its plus button exposes only the existing image path and text-readable files up to 256 KB; text, Markdown, CSV/JSON, and common code are inserted into the draft and persist with the ordinary message, without a second file store or filesystem authority. It still preserves ordinary model selection, durable history, titles, search, and deletion behavior. A message that starts a turn is echoed into either chat or work conversation immediately, before the turn activity label, then handed over to the durable Host event without duplicating the bubble.
...
The plugin center is itself a native plugin. Every curated capability declares its category, package roots, and Cordis rows once; groups, counts, search results, runtime switches, and export choices derive from that catalog. **Continuous Adaptation** is included because it is an independent capability; the composer add menu is deliberately excluded because it is baseline product interaction rather than a user-facing plugin. **Export plugins** enters selection in the current list, while **Select all 16** means the complete catalog even when search hides rows or a plugin is disabled. The selection count stays plain text, with only the final export action styled as a primary button. The Host creates the ZIP in memory, includes only source, built code, and declared runtime assets, and adds `AGENTS.md`, `INSTALL.md`, and a SHA-256 manifest so another AI can install directly or adapt narrow conflicts against its target DSH version. No conversation, setting, credential, `node_modules`, cache, test tree, or superseded companion asset is included.
...
Settings exposes user memory and AI memory as two separate global Markdown documents. User memory changes only through explicit selection or direct editing. At local midnight, a continuously running DSH reviews only the calendar day that just ended, loading one conversation at a time, and maintains its own document by adding, merging, updating, and deleting knowledge. Startup never scans history, and a missed midnight is not replayed later. Subsequent requests receive only a few matching blocks immediately before the current request, inside an explicit untrusted-data boundary.
