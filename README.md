...
The sidebar separates intent before a conversation starts: **Start work** follows the current Workspace flow, while **Start chat** opens or reuses a blank conversation under a dedicated **Chats** group. If the user changes their mind while either start is still resolving, the latest click wins instead of an older asynchronous result taking the page back. Chat uses an internal no-Tool preset, is not attached to a folder, and omits Workspace, mode, add-capability, and permission controls. It still preserves ordinary model selection, durable history, titles, search, and deletion behavior. A message that starts a turn is echoed into either chat or work conversation immediately, before the turn activity label, then handed over to the durable Host event without duplicating the bubble.
