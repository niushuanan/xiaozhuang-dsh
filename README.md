...
| **Native Skill management** | Browse every personal, project, runtime, custom, and bundled Skill, inspect its directory and files inline, and adaptively import files, folders, ZIP archives, or a GitHub repository into personal Skills. |
...
The native **Skill Management** Settings page turns the existing registry into a readable library. The left side lists every visible Skill with the existing Skill glyph and source label; opening one shows its human description, directory tree, and selected file in the same page. When pure Chat is selected, management reads the latest work Session's catalog instead of hiding the user's installed Skills; Chat itself still receives no Skill or Tool capability. Personal Skills are writable while project, runtime, custom, and bundled sources remain read-only. Import accepts local files, folder selections, ZIP archives, and GitHub repository URLs. Sources are staged as untrusted data, normalized by `deepseek-official/deepseek-v4-flash-vision-exp`, validated, and atomically installed only into the personal Skill directory, so a failed adaptation leaves the previous Skill intact.
