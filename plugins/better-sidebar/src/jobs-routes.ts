 ...
 */
import { snapshotSidebarSessionEvents, type Context, type SidebarSessionEvent } from './context-types.ts'
 ...
 * boundary, so its event snapshot misses everything appended since — the very
...
export function buildJobsApi(ctx: Context, outputLimit: number): SidebarJobsRoutes {
  ...
  return {
    output(payload) {
      ...
      for (const event of snapshotSidebarSessionEvents(ctx.sessions.get(sessionId))) {
        const trace = traceOf(event)
