...
/** Directory holding the built client chunk scripts (`lib/` next to this
 *  package's package.json). Resolved through the package's own
 *  `./package.json` export so the path stays correct whether this module runs
 *  from `src/` (tests) or the bundled `lib/index.js`; falls back to the
 *  module's own directory when the package cannot be self-resolved. */
function resolveLibDir(): string {
  try {
    return join(dirname(require.resolve('@deepseek-ai/dsh-better-sidebar/package.json')), 'lib')
  } catch {
