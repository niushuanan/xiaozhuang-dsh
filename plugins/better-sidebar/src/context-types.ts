...
export interface SidebarSessionStore {
  get(id: string): {
    ...
    /**
     * Return the live session's immutable event-log snapshot. This is the
     * public rc.7+ read boundary; the legacy property remains optional so a
     * plugin test double or older host can still degrade safely.
     */
...
/** Read one host session through the current public API, with a legacy
 * compatibility fallback for older hosts and lightweight test doubles. */
export function snapshotSidebarSessionEvents(
  session: ReturnType<SidebarSessionStore['get']>,
