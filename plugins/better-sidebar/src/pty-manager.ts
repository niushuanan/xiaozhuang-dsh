/**
 * PTY session table for the sidebar terminals. One terminal process per
 * `${sessionId}:${tabId}` key, spawned through the host subprocess seam
 * (`ctx.subprocess.spawnTerminal`) instead of node-pty. Processes survive
 * WebSocket disconnects (page refresh, tab switch) and reconnect to the same
 * process by key. Output is mirrored into a bounded transcript ring (capped
 * bytes) so a new connection replays history before live data. Sessions die
 * only when the tab is closed or the plugin tears down.
 */
...
export interface SidebarPty {
  ...
  cwd: string
  /** The live subprocess terminal handle (UTF-8 output stream + write/signal/terminate). */
...
/** Append one output chunk's UTF-8 text to a transcript, keeping only the tail. */
function appendTranscript(holder: { transcript: string }, chunk: Buffer | string): void {
  holder.transcript += typeof chunk === 'string' ? chunk : chunk.toString('utf8')
  if (holder.transcript.length > TRANSCRIPT_LIMIT) {
    holder.transcript = holder.transcript.slice(holder.transcript.length - TRANSCRIPT_LIMIT)
  }
}
...
export class PtyManager {
  ...
  async open(
    ...
    // The handle's `output` is a Readable; its 'data' callback owns the
  ...
  close(key: string): void {
    ...
    // terminate() is idempotent and owns whole-session quiescence; the
    // transcript stays replayable through the surviving output stream.
 ...
 * @param options - platform / environment / explicit-shell / existence-probe overrides for deterministic resolution.
 * @returns the resolved interactive shell executable.
 */
 ...
 * @returns a short display name for a terminal tab title.
 */
 ...
 * @param configured - explicit shell arguments; empty keeps the platform defaults.
 * @returns the argv suffix handed to the shell executable.
 */
