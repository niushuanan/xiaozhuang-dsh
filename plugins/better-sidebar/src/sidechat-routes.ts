...
/** Read a live Agent session through the rc.7 public snapshot API while
 * retaining compatibility with the lightweight legacy-shaped test doubles. */
function liveSessionEvents(session: Agent['session']): readonly SidechatLogEvent[] {
  const readable = session as unknown as {
    snapshotEvents?(): readonly SidechatLogEvent[]
    events?: readonly SidechatLogEvent[]
  }
  return readable.snapshotEvents?.() ?? readable.events ?? []
}
...
export function buildSidechatApi(ctx: Context): SidechatRoutes {
  return {
    'sidechat.start': async (payload: unknown) => {
      ...
      const parentEvents = liveSessionEvents(parentSession)
      const inheritance = buildSidechatInheritance(
        parentEvents as unknown as readonly SidechatLogEvent[],
      )
    ...
    'sidechat.prompt': async (payload: unknown) => {
      ...
      if (boundaryDelivered(liveSessionEvents(agent.session))) {
