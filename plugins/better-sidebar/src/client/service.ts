...
/**
 * Consumer-facing augmentation restored on the client half (the host half
 * dropped it because the host face cannot resolve this React-typed file): a
 * plugin that does `import type {} from 'dsh-better-sidebar'` and reads the
 * vendored `@deepseek-ai/cordis` Context sees `ctx.betterSidebar` without
 * importing this package's own Context type. The augmentation also flows
 * through the shared `../context-types.ts` `Context` intersection inside the
 * client face (its `CordisContext` base is the augmented vendored Context).
 */
