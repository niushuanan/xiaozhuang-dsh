...
describe('side card preferences', () => {
  ...
  it('defaults browser input to URL mode and accepts only the two supported modes', async () => {
    expect((await loadPrefs(wire({}))).browserDefaultMode).toBe('url')
    expect((await loadPrefs(wire({ browserDefaultMode: 'search' }))).browserDefaultMode).toBe('search')
    expect((await loadPrefs(wire({ browserDefaultMode: 'url' }))).browserDefaultMode).toBe('url')
    expect((await loadPrefs(wire({ browserDefaultMode: 'smart' }))).browserDefaultMode).toBe('url')
  })
