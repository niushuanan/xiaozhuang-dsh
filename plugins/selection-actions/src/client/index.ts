...
import { captureDshSelection, type DshSelectionPacket } from './selection.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.slots.inject('shell.overlay', () => ctx.slots.register({
    ...
    locale: NS,
    inject: () => {
      const auxiliaryPane = ctx.get('auxiliaryPane') as AuxiliaryPaneOpener | undefined
      return {
        capture: () => {
      ...
      quote: async (packet: DshSelectionPacket) => addSelectionQuote(ctx, packet),
      ...(auxiliaryPane === undefined ? {} : {
        sideChat: async (packet: DshSelectionPacket) => (await openSelectionQuote(ctx, packet, auxiliaryPane)).pane,
      }),
      remember: async (packet: DshSelectionPacket) => {
        try {
