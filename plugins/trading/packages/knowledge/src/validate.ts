...
export function validateKnowledgeCard(
  ...
  const card: KnowledgeCard = {
    ...
    ...(input.tickers === undefined ? {} : { tickers: input.tickers.map((t) => String(t).trim().toUpperCase()).filter(Boolean) }),
    ...(input.related === undefined ? {} : { related: Array.from(new Set(input.related)) }),
    createdAt,
    updatedAt,
  }
