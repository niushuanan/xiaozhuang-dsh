...
import type { UseStoreState } from './StrategyView.tsx'
...
interface StageViewsService {
  register(definition: {
    ...
    titleKey: string
    order?: number
    render: ComponentType<{ t: (key: string) => string; view: string; useSelection: UseStoreState<SelectionState> }>
  }): void
}
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.inject(['tradingStageViews', 'tradingBridge'] as never, (scope) => {
    ...
    const bridge = {
      fetchKlines: (market: string, symbol: string, interval: string, limit: number) =>
      ...
      fetchSymbols: (market: string) =>
        faces.tradingBridge.fetchSymbols(market) as never,
      subscribeTradingEvents: (handlers: { strategies?: () => void }) => faces.tradingBridge.subscribeTradingEvents(handlers),
