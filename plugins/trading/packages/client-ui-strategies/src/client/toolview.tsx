    ...
    <div className={css.card} data-dshtrading-toolview="strategy-backtest">
      ...
      <div className={css.metrics}>
        {METRIC_CELL.map(cell => {
          const value = payload[cell.key] as number | null
          return (
          <div key={cell.key} className={css.metric}>
            ...
            <span
              ...
              data-tone={(cell.key === 'totalReturn' || cell.key === 'cagr') && value !== null ? value >= 0 ? 'up' : 'down' : undefined}
            >
              {fmt(value, cell.suffix ?? '', cell.plus === true)}
            </span>
          </div>
          )
        })}
