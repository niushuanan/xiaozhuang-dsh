...
export interface CnFundamentalsResult {
  ...
  amplitudePercent?: number | undefined
  limitUpPrice?: number | undefined
  limitDownPrice?: number | undefined
...
export async function fetchCnFundamentals(options: CnFundamentalsOptions): Promise<CnFundamentalsResult> {
  ...
  if (hithinkApiKey) {
    try {
      ...
      const hithinkFund = await client.getStockFundamentals(canonical)
  ...
  try {
    ...
    const groups: import('@dshtrading/api').FinancialReportGroup[] = [
      ...
      },
      {
        ...
        title: '盈利能力',
        rows: [
          makeRow('gross_margin', '销售毛利率', 'XSMLL', 'XSMLL_TB', '%'),
          makeRow('net_margin', '销售净利率', 'XSJLL', undefined, '%'),
          makeRow('roe', '净资产收益率 (ROE)', 'ROEJQ', 'ROEJQTZ', '%'),
          makeRow('roa', '总资产收益率 (ROA)', 'ZZCJLL', undefined, '%'),
        ],
      },
      {
        id: 'growth',
      ...
      },
      {
        ...
        title: '现金流与偿债',
        rows: [
          makeRow('debt_to_asset_ratio', '资产负债率', 'ZCFZL', 'ZCFZLTZ', '%'),
        ],
      },
    ]
