...
export function createTasksCreateTool(service: TradingTasksService) {
  return defineTool({
    ...
    description:
      ...
      + '缺省落插件默认：agentPreset=trading-master（大师）、permission=workspace-write（有写入权限，启动时应用）。'
    ...
    parameters: {
      ...
      agentPreset: { type: 'string', description: '执行会话的 agent 预设 id（tasks_meta 查名册）；缺省 = trading-master（大师）' },
