...
/**
 * Adapt task session identities to the native command runtime's Agent argument.
 * @param agents - Host registry containing the newly created execution session.
 * @param commands - Host command service, which logs the permission command result.
 * @returns The task runner's narrow session command dispatcher.
 */
export function createNativeCommandDispatcher(
  ...
  return {
    async execute(sessionId, line, signal) {
      const agent = agents.get(sessionId as Parameters<AgentRegistry['get']>[0])
      if (agent === undefined) throw new Error('execution session is unavailable: ' + sessionId)
