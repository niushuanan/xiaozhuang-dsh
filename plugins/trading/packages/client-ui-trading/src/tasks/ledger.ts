...
interface LedgerDocument {
  ...
  recentRequests: Array<{ requestId: string; serializedAction: string }>
}
...
function serializedActionOf(action: TasksAction): string {
...
export class TasksLedger {
  ...
  private load(): LedgerDocument {
    ...
    return {
      ...
      recentRequests: Array.isArray(row.recentRequests)
        ? (row.recentRequests as Array<{ requestId?: unknown; serializedAction?: unknown }>)
          .filter(item => typeof item?.requestId === 'string' && typeof item?.serializedAction === 'string')
          .map(item => ({ requestId: item.requestId as string, serializedAction: item.serializedAction as string }))
        : [],
    }
  }
  ...
  apply(envelope: TasksActionEnvelope): AppliedAction {
    ...
    const serializedAction = serializedActionOf(envelope.action)
    const cached = this.document.recentRequests.find(item => item.requestId === envelope.requestId)
    if (cached !== undefined) {
      if (cached.serializedAction !== serializedAction) {
    ...
    const now = this.now()
    const next = JSON.parse(JSON.stringify(this.document)) as LedgerDocument
    next.recentRequests.push({ requestId: envelope.requestId, serializedAction })
