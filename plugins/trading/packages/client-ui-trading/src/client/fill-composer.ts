...
export interface ConversationDraftFace {
  createDrafts(sessionId: Parameters<ISessions['open']>[0], files: readonly File[]): ReadonlyArray<{ id: DraftAttachmentId }>
...
export async function fillComposerWithQuote(deps: FillComposerDeps, text: string, image?: SendImageInput): Promise<void> {
  ...
  if (image !== undefined) {
    const [attachment] = conversation.createDrafts(current, [dataUrlToFile(image.dataUrl, image.name ?? 'chart.png')])
