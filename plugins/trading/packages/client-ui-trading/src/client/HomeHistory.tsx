...
function mostRecentlyActive(rows: WorkspaceRow[], byId: SessionListState['byId']): string | undefined {
  ...
  for (const row of rows) {
    ...
    for (const id of row.sessionIds) {
      latest = Math.max(latest, byId[id as keyof SessionListState['byId']]?.updatedAt ?? Number.NEGATIVE_INFINITY)
    }
...
export function HomeHistory({ t, useSessions, useWorkspaces, openSession, startNewSession, renameSession, forkSession, archiveSession, deleteWorkspace }: HomeHistoryProps) {
  ...
  const commitRename = (): void => {
    ...
    setRenaming(null)
    if (next === '' || next === sessions.byId[target as keyof SessionListState['byId']]?.displayTitle) return
  ...
  const historyRows = (workspaceRows.find(row => row.workspaceId === scopeWorkspace)?.sessionIds ?? [])
    .map(id => sessions.byId[id as keyof SessionListState['byId']])
    .filter((row): row is SessionListState['byId'][keyof SessionListState['byId']] => row !== undefined && !row.blank && row.origin !== 'subagent' && !archivedIds.has(row.id))
