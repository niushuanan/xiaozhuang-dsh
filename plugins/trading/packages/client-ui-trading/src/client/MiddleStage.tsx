...
import { QuoteStage, type QuoteStageProps } from './QuoteStage.tsx'
...
export function MiddleStage({ t, useSelection, useChart, toggleIndicator, setIndicatorParams, setIndicatorVisible, removeIndicator, deleteIndicator, fillComposer }: MiddleStageProps) {
  ...
  return (
    <div className={css.root} data-dshtrading-middle-stage="">
      <div className={css.tabs} role="tablist" aria-label={t('workspace.sections')}>
      ...
      {view === 'quote' ? (
        <QuoteStage {...({ t, useSelection, useChart, toggleIndicator, setIndicatorParams, setIndicatorVisible, removeIndicator, deleteIndicator, ...(fillComposer === undefined ? {} : { fillComposer }) } as QuoteStageProps)} />
      ) : (
        (() => {
