...
export function applyColorModeToRoot(mode: ColorMode): void {
  ...
  for (const surface of document.querySelectorAll<HTMLElement>('[data-dshtrading-surface]')) {
    surface.style.setProperty('--dsw-futu-up', palette.upColor)
    surface.style.setProperty('--dsw-futu-down', palette.downColor)
    surface.dataset.dshtradingColorMode = mode
  }
}
