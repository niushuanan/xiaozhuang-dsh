      ...
      <section className={css.card}>
        ...
        <dl className={css.rows}>
          {([
            ...
            ['topLongShort', derivatives?.topTraderLongShortRatio, t('derivatives.hint.topLongShort')],
            ['taker', derivatives?.takerBuySellRatio, t('derivatives.hint.taker')],
          ] as Array<readonly [string, number | undefined, string]>).filter((row): row is readonly [string, number, string] => row[1] !== undefined).map(([key, ratio, hint]) => (
