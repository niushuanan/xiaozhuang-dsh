...
import { createNativeCommandDispatcher, type SessionGateway } from './tasks/runner.ts'
  ...
  ctx.inject(['webServer', 'connection'], (webCtx) => {
    ...
    try {
      tasksService = new TradingTasksService({
        ...
        commands: () => {
          const agents = resolveHostService('agents') as AgentRegistry | undefined
          const commands = resolveHostService('commands') as CommandRuntime | undefined
          return agents === undefined || commands === undefined ? undefined : createNativeCommandDispatcher(agents, commands)
        },
