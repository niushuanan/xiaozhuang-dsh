...
describe('TasksLedger', () => {
  ...
  it('create 未钉住预设/权限时落插件默认：大师 + workspace-write，且不触发确认门', () => {
    ...
    expect(task?.agentPreset).toBe('trading-master')
  ...
  it('create 显式空串 agentPreset 同样落大师默认', () => {
    ...
    expect(env.ledger.snapshot().tasks.find(item => item.id === 't-empty-preset')?.agentPreset).toBe('trading-master')
  })
  ...
  it('同 requestId 同动作内容幂等重放；不同动作内容拒绝', () => {
