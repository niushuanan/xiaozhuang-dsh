...
import { extractBaseAsset } from './derivatives.js'
...
export interface CryptoFundamentalsResult {
  data?: CryptoFundamentals | undefined
  quoteVolume24h?: number | undefined
  priceChangePercent24h?: number | undefined
...
export function renderCryptoFundamentals(result: CryptoFundamentalsResult, requestedSymbol: string): string {
  const { data, priceChangePercent24h, tradesCount24h, unavailable = [] } = result
  if (!data) {
