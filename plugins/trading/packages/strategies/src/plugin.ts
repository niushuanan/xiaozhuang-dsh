...
export function createStrategyBacktestTool(deps: StrategyBacktestToolDeps) {
  return defineTool({
    ...
    async execute(raw) {
      ...
      // Each provider validates its supported canonical interval before fetching.
      const interval = (typeof args.interval === 'string' && args.interval.trim() ? args.interval.trim() : '1d') as Interval
