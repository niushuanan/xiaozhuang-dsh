...
**唯一权威存储**：`$DSH_HOME/trading/knowledge/cards.json`（未设置 `DSH_HOME` 时为
`~/.dsh/trading/knowledge/cards.json`）——宿主交易知识库 UI 的唯一数据源。markdown 知识卡片（S3 产物）只是过程底稿；**不入库 = 未沉淀**。
...
| knowledge_ingest 报 store.list is not a function | 会话绑定了旧构建或 dsh 进程未重载：检查当前宿主加载的交易插件版本与服务接线；批量任务用 S4 直写回退协议 |
