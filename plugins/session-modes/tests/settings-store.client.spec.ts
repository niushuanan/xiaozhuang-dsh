...
type SettingsWireFace = Pick<ClientRemote, 'settings'>
...
function derivedController(wire: FakeWire) {
  const remote = { ...wire.remote, ...wire.api } as Pick<ClientRemote, 'agentPresets' | 'settings'>
  return new AgentPresetSettingsController(
    remote,
    new SettingsDescribeMirror({ remote } as never),
  )
}
