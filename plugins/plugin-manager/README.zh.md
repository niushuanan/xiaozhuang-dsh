...
**导出插件**会在当前目录直接进入选择模式。用户可以逐项选择，也可以全选目录中的 17 项能力；选择条只保留纯文字数量，只有最终导出动作使用主按钮。全选不受搜索与当前启停状态影响：已经安装但处于关闭状态的能力仍可导出。目录包只是服务这些插件的基础能力，不会把自己列进导出项。**持续适配**是独立能力，因此进入目录并可导出。输入框添加菜单属于常驻的产品基础交互，刻意不单独进入可开关、可导出的插件目录。**聊天模式**负责无工作区聊天入口、附件菜单与内部无工具预设；**导入对话**负责 DeepSeek 历史预览、选择和原生会话迁移。两项能力在目录中独立启停、独立导出，导出包各自带齐直接依赖；此前已打开页面使用的 `plain-chat`／`chat-migration` 开关请求仍会兼容映射到新目录项。**Skill 管理**会导出自己的原生包源码，让另一套 DSH 通过普通 Cordis 行恢复 Skill 能力库。
