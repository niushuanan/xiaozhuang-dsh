...
describe('native plugin catalog controls', () => {
  ...
  it('reports compound capability state from every mapped Loader row', () => {
    ...
    expect(PLUGIN_ROWS['conversation-import']).toEqual(['conversation-import'])
    expect(PLUGIN_ROWS['session-modes']).toEqual(['session-modes'])
    ...
    expect(PLUGIN_EXPORT_CATALOG['chat-mode']?.sources).toEqual([
      { kind: 'product', path: 'plugins/chat-mode' },
    ])
    ...
    expect(PLUGIN_EXPORT_CATALOG['conversation-import']?.sources).toEqual([
      { kind: 'product', path: 'plugins/conversation-import' },
    ])
    expect(PLUGIN_EXPORT_CATALOG.teamwork?.sources).toEqual([
      { kind: 'product', path: 'plugins/teamwork' },
    ])
  ...
  it('keeps switches usable for an already-open legacy catalog page', async () => {
    ...
    expect(chatMigration.patch).toContain('- id: conversation-import\n  disabled: false')
  })
