...
/**
 * Render the user-global AGENTS.md as the single highest DSH-owned authority.
 * The returned block is self-describing because providers receive one flat
 * system string even though the registry retains section attribution.
 * @param file - fixed user-global instruction file.
 * @param maxBytes - maximum UTF-8 bytes contributed to the system prompt.
 * @returns protected owner-directive text, or an empty string when disabled.
 */
