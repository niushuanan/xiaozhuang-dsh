...
An entry with `hidden: true` is omitted from that advertisement while everything else about it keeps serving: exact-route resolution still returns its capacity metadata, so a selector cannot offer the model but an existing selection or an explicit reference keeps working. Hiding is therefore reversible without retyping the entry — flip the flag back and the catalog face lists it again.
...
- **Text-only image projection depends on the local vision tool** — text-only DeepSeek models receive `image_vision` attachment notes; image-capable models use provider-native input. Other plugin-added block types are skipped, and empty tool output crosses the wire as the literal `(no output)`.
