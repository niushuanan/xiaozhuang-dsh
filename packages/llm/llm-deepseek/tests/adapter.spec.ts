...
describe('DeepSeekAdapter against a mock server', () => {
  ...
  it.each(['deepseek-v4-flash', 'unlisted-pass-through'])(
    'projects image input for text-only model %s into a local vision-tool note',
    async (model) => {
      const server = await mockServer([{ kind: 'sse', events: textEvents }])
      ...
      expect(server.requests[0]).toMatchObject({
        ...
        messages: [{
          role: 'user',
          content: `【图片 attachment:${imageRef.attachmentId} —— 请用 image_vision 工具查看这张图片】`,
        }],
      })
...
describe('plugin registration and config', () => {
  ...
  it('omits hidden catalog entries from the advertisement while keeping them resolvable', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmDeepSeek, {
      baseURL: 'http://127.0.0.1:1',
    ...
    // Hiding is advertisement-level: the exact route still answers with its
    // configured capacity metadata, so an existing selection keeps working.
