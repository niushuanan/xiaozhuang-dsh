...
所选 DeepSeek 模型会收到 harness 系统提示词、消息历史、工具 schema、stop sequence 和调用配置。视觉模型会通过 base64 data URL 收到保留的 user 与工具结果图片；超出上限的较旧图片由已记录的占位文本表示。在本地部署中，纯文本 DeepSeek 模型则会收到 `【图片 attachment:<id> —— 请用 image_vision 工具查看这张图片】`，让已安装的 `image_vision` 工具解析该持久附件。之前 assistant 轮次的推理内容会原文回传，无论该轮次是否调用了工具。
