...
配置了 `hidden: true` 的条目不会出现在上述公布中，其余行为全部保持：精确路由解析仍返回其容量元数据，因此选择器不再提供该模型，但既有选择或显式引用仍然可用。隐藏因此是可逆的——把标志翻回去，catalog 面就会重新列出它。
...
所选 DeepSeek 模型会收到 harness 系统提示词、消息历史、工具 schema、stop sequence 和调用配置。视觉模型通常通过 Files API 引用收到保留的 user 与工具结果图片，旁边带有稳定附件句柄和请求图片尺寸；Files 解析失败时，所有保留图片改用内联 data URL。超出上限的较旧图片由已记录的占位文本表示。在本地部署中，纯文本 DeepSeek 模型则会收到 `【图片 attachment:<id> —— 请用 image_vision 工具查看这张图片】`，让已安装的 `image_vision` 工具解析该持久附件。之前 assistant 轮次的推理内容会原文回传，无论该轮次是否调用了工具。
...
- **纯文本图片投影依赖本地视觉工具**：纯文本 DeepSeek 模型会收到 `image_vision` 附件注记，支持图片的模型则使用提供方原生输入。其他插件添加的块类型会被跳过，空工具输出会以字面 `(no output)` 通过协议发送。
