...
export class PiAiAdapter extends LlmAdapter {
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    try {
      ...
      const nativeImageInput = containsImage && model.input.includes('image')
      const attachments = nativeImageInput ? this.config.resolveAttachments?.() : undefined
      if (nativeImageInput && attachments === undefined) {
