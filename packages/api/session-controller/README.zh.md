...
Session 对象还承载本地提交回显：`session.beginSubmission` 在调用方序列化与 prompt 之前，同步把一条回显写入 `SessionSnapshot.pendingSubmissions`，会话 UI 因此能在点击提交的当帧显示消息。prompt 的 `requestId` 就是关联标识，Host 本就把它回显为 durable user source 的 `rpcId`，queue occurrence 也把它投影为 `SessionQueuedItem.rpcId`。普通空闲会话的提交会一直保留回显，直到 durable event 到达；只有在已有 Turn 运行时发出的提交，才允许在观察到 queue occurrence 后退休。这个区分避免空闲提交短暂经过 inbox 时，唯一可见消息先于正式 transcript 节点消失。退休仍延迟一个动画帧；带标识的 prompt 失败或被放弃时立即退休，销毁时按 failed 退休；每次退休恰好触发一次注册的 `onRetire` 回调。回显只存在于 Client 内存，刷新与重连只从 durable event 重建会话。
