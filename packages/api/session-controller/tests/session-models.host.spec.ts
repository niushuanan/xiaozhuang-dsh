...
describe('Web session model selection', () => {
  it('guards web search for a disabled prompt and lifts the guard when re-enabled', async () => {
    ...
    expect(policy({ name: 'web_search' })).toMatch(/disabled by the user/u)
    expect(policy({ name: 'web_fetch' })).toMatch(/disabled by the user/u)
