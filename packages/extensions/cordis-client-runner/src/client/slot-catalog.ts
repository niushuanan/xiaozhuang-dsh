...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'settings.section.icon',
    kind: 'list',
    scope: 'root',
    summary: 'Optional navigation glyph, matched by the settings.section entry id.',
    doc: 'Optional navigation glyph, matched by the settings.section entry id.\nThe feature owns its glyph and registration lifetime. Missing entries\nuse the shell\'s default icon without affecting section availability.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'settings.section.icon\', () => ctx.slots.register(\n      { name: \'settings.section.icon\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  {
    key: 'shell.documentTitle',
    kind: 'single',
    scope: 'root',
    summary: 'Browser title projection; the layout supplies the selected Session title.',
    doc: 'Browser title projection; the layout supplies the selected Session title.',
    registerOptions: [],
    ownerProps: [
