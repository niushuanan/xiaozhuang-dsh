...
`cachedSnapshot(meta, inheritedEventCount)` synchronously serves client values from the storage domain's in-memory tables with zero I/O. It accepts only an identity-matching record and version- and schema-matching keys, then returns a `{ asOfSeq, values }` cut at the lowest served-row watermark. `cachedPredecessorSnapshot(meta, inheritedEventCount, keys)` is the narrower listing-only exception: a caller may explicitly request navigation hints whose meaning it knows is stable across the adjacent Session format. Each requested row must still pass the current projection version and schema. The result is a possibly stale fact from a durable prefix, never a fold seed, and carries the sentinel `asOfSeq: -1` because a cardinality-changing migration can invalidate predecessor sequence numbers. `cachedPredecessorTitle(...)` is the title-only convenience wrapper. Unrequested predecessor rows remain unavailable.

`cachedNavigationSnapshot(meta, keys)` serves only explicitly requested display values when a header-only listing lacks the inherited cut, including old forks. The cache key, creation time, and working directory must match; a stored seeded flag must agree when present, and a future Session format refuses. Missing legacy lineage fields are accepted only for this navigation read. Each row still passes the current projection version and wire schema, and the result always carries `asOfSeq: -1`. This method never supplies a fold seed or a numeric inherited cut. `coldSnapshot(meta, inheritedEventCount, events)` accepts the exact cut with a complete ordered log, skips the checkpointed prefix while folding, and refreshes the record without reading persistence itself.
