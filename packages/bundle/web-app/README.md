...
The browser roster mounts [`ui-chat`](../../client/ui-chat/README.md) after the sidebar declaration and supplies the internal `chat` agent preset. This makes Start chat a native optional plugin contribution while preserving Start work and the shared Session runtime. The same roster mounts [`ui-skill-manager`](../../client/ui-skill-manager/README.md) as the native Skill Management Settings page and [`ui-composer-add-menu`](../../client/ui-composer-add-menu/README.md) after `ui-conversation`, so the add owner receives the live Session command and Skill catalogs rather than duplicating their protocols.
