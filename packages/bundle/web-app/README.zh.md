...
浏览器名录会在侧边栏声明之后挂载 [`ui-chat`](../../client/ui-chat/README.zh.md)，并提供内部 `chat` agent 预设。因此开始聊天是原生、可选的插件贡献，开始工作和共享 Session 运行时保持不变。同一名录还把 [`ui-skill-manager`](../../client/ui-skill-manager/README.zh.md) 挂成原生 Skill 设置页，并在 `ui-conversation` 之后挂载 [`ui-composer-add-menu`](../../client/ui-composer-add-menu/README.zh.md)，让添加入口直接接收当前会话的命令和 Skill 目录，而不是复制它们的协议。
