...
展开的品牌行把 `sidebar.brand.mark` 与 `sidebar.brand.name` 渲染为两个独立的 single slot，收起轨道则渲染同一个 mark slot。没有占位者时，外壳仍保留官方鱼形标记，以及原始 DeepSeek 与 `HARNESS` 字标图形；不会用普通文字重新拼装品牌，也不会把构建 hash 塞进 logo。部署包可以单独替换任一值，而无须替换 New Session 控件或轨道几何；声明感知的 `slots.inject()` 让这种包无论先于还是后于侧边栏激活都能生效。
