...
Unified Web `@file` and `@session` source. The browser starts the `fileReferences/list` and `sessionReferenceResolver/candidates` Remote calls together for an ordinary unquoted `@` token, deterministically orders files before sessions with locale-registered folder/file/session labels, and renders the rows under non-selectable file and session section headings without a redundant raw `reference` source title. Either failed candidate domain degrades independently. An open `@"…` token searches files only. The composer's explicit **File & folder** launcher is also workspace-only: it skips session enumeration entirely, so that direct action opens promptly and matches its label while typed `@` retains the richer cross-session reference flow.
