...
One `register()` call contributes `AppFrame` into the runtime's built-in `'root'` slot and declares the child slots (`sidebar`, `conversation`, `details`, `shell.overlay`, `shell.documentTitle`), seats the layout store (panel geometry), and wires the `ctx.layout` panel-action service. The transient layout store starts the sidebar at its default width and details closed, and never reads or writes `localStorage`. AppFrame always mounts the conversation and details columns; a connected Session renders through `SessionProvider`. The optional `shell.documentTitle` occupant receives the selected Session title and owns browser title projection. Without an occupant, the frame combines that Session title with the build-configured product title or localized `common.brand.localBuild` fallback, including after an occupant unloads. The theme presenter is a second effect: pure DOM writes from resolved snapshots — initial state through the getter once, then event-driven only, with no React path. It applies palette, font-size, and token variables before measuring the rendered background as the single color authority.
