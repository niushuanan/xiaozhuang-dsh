...
Native cross-page product companion for Xiaozhuang DSH. The character is named **Lanxi**, after ocean swells and tides: a concise identity for the DeepSeek-blue, whale-inspired adult anime character rather than a generic pet label. The plugin contributes one additive entry to `shell.overlay`, so Lanxi stays mounted while the user switches conversations, Settings sections, and product pages. It reads the existing session-list projection and selects a generated frame without making another model request:
