...
An unauthorized bare-root GET returns HTTP 401 HTML that navigates once to `/?reconnect=1`. This same-site document navigation lets a reopened desktop window send its saved `SameSite=Strict` cookie; a valid cookie redirects to clean `/` without minting a new cookie. If authentication still fails, navigation stops at the same-origin launch-token form. Query-bearing and configured-index failures show the form without automatic navigation. The response never embeds the token, disallows external form destinations, and uses `no-store` and `no-referrer`. Submitting the token uses the existing root exchange; HEAD has no body. Failed API requests remain unauthorized.
