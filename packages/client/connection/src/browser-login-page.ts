 ...
 * @param reconnect - whether to retry once as a same-site navigation for a saved Strict cookie.
 * @returns static HTML containing no request values or authentication secrets.
 */
export function browserLoginPage(chinese: boolean, reconnect = false): string {
  const t = labels[chinese ? 'zh' : 'en']
  return `<!doctype html><html lang="${chinese ? 'zh-CN' : 'en'}"><head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
${reconnect ? '<meta http-equiv="refresh" content="0;url=/?reconnect=1">' : ''}
