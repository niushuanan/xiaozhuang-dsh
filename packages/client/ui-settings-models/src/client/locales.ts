...
export const en = {
  ...
  modelVision: 'Native vision',
  modelVisibility: 'Model visibility',
  modelVisible: 'Shown in the selector. Click to hide it.',
  modelHidden: 'Hidden from the selector; direct requests still work. Click to show it again.',
  modelCapabilityHint: 'Text-only models use image_vision for images; native-vision models receive images directly. New models default to text-only. The eye on each row hides a model from the selector without deleting it.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  modelVisible: '已展示在选择器中，点击隐藏。',
  modelHidden: '已从选择器隐藏；直接指定仍可调用。点击恢复展示。',
  modelCapabilityHint: '纯文本模型遇到图片时调用 image_vision；原生视觉模型直接接收图片。新增模型默认归为纯文本。每行的眼睛图标可以把模型从选择器隐藏而不删除配置。',
  contextWindow: '上下文窗口',
