...
按住任意设置目录一秒即可直接调整导航顺序。拖动期间只显示一条高对比度插入线；指针越过尚未占满的目录上下边界时，目标会限制为第一个或最后一个真实目录，不会把面板空白算成额外位置。松手后，外壳的根级查看状态会保存完整分区 id 顺序，因此重新打开设置或刷新页面仍保持当前排列。之后新注册的分区会按插件原有顺序追加，已移除的分区 id 会直接消失，不影响其余排列。普通单击仍立即切换分区，不会进入额外编辑模式。
