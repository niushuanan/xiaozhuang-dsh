...
会直接影响所选会话此后的工作：下一轮使用该 preset 的工具、技能、命令与提示词段落。具体组装由 [`dsh-agent-presets`](../../preset/agent-presets/README.zh.md) 负责；当前轮次与既有对话记录不会被改写。

#### KV Cache effect

更改部署默认值绝不触及运行中会话。切换既有会话会从下一轮开始改变模型可见前缀，因此不期待跨越组装边界复用缓存；当前请求保持不变。
