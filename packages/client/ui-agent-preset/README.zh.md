...
agent preset 的各个表层：General 设置中的一行，用于选择新建会话据以组装的 [preset](../../preset/agent-presets/README.zh.md)；新建会话界面上的一枚 chip，用于选择**下一个会话**的 preset；会话标题旁的下一轮模式切换器；以及一个设置页分区，用于管理名单——复制、删除、默认值，以及通往 preset 自身文件的入口。

## 新会话的默认值

General 行只设置部署默认值，不会暗中改写既有对话；需要不同组装时，用户通过对应会话自己的标题栏控件切换。
...
已经开始的会话交给标题栏控件处理。新会话暂存值仍只会被空白会话消费，从而保持“一个界面、下一个会话”的单一含义。

## 会话标题旁的切换器

第三个表层，位于会话标题旁：既显示**本会话**所运行的 preset，也提供一个菜单来切换下一轮使用的组装。它从会话自身的摘要读取当前 preset，并在 General 行所读的同一份名单上解析所有显示名称。

空闲会话立即切换。当前轮次正在运行时，选择值会保存在按会话隔离的插件 store 中，并标注“下轮生效”；当前任务继续使用原来的工具与提示词，等会话列表下一次报告空闲后再提交切换。Host 会为重链占用一个空闲 maintenance 阶段，新到达的唤醒输入也会等在切换之后。如果另一个维护任务先占住该边界，`agent-preset-locked` 会让选择继续排队到下一次空闲更新，而不是变成用户可见的失败。

已提交的 `agent-preset/selected` owner 事件会在每个标签页把新 id 折进共享摘要，并清掉匹配的排队状态。发起方标签页可能已经采用 RPC 回执，而合并是幂等的。
...
删除会移除整个 preset 目录。已经连接到常驻组装的活跃会话仍可继续运行，但该 preset 此后无法再被选择，进程重启后也无法据其重建。
...
Indirectly, through 所选 preset 的工具、技能、命令与提示词段落，在所选会话的下一轮生效；具体组装由 [`dsh-agent-presets`](../../preset/agent-presets/README.zh.md) 负责，当前轮次与既有对话记录不会被改写。

#### KV Cache effect

切换既有会话会从下一轮开始改变模型可见前缀，因此不期待跨越组装边界复用缓存；当前请求保持不变，更改部署默认值也绝不触及运行中会话。
