...
- **Older history loads progressively** — after a Session opens, Chat requests every preceding event page in sequence so Turn navigation grows to represent the complete available conversation without requiring repeated `Load earlier` actions. Existing Turn marks keep their identity while an earlier page is in flight and redistribute as pages prepend. Marks stay 10px apart until the complete set exceeds the available height, then compress to fit. If a Host history request stalls or fails, Chat keeps the currently visible marks and waits for the next Session open or resync instead of retrying in a hot loop.
