...
- **更早历史会渐进加载**——Session 打开后，Chat 会依次请求所有更早的 event 页，因此无需反复点击「加载更早」，轮次导航便会逐步覆盖整段可用对话。请求前一页期间，已有 Turn 刻度保持身份不变；新页前插后，完整刻度集合重新排布。刻度默认相隔 10px，仅在完整集合超过可用高度时压缩间距。若 Host 的历史请求卡住或失败，Chat 会保留当前可见刻度，等待下次 Session 打开或重新同步后再继续，不进行高频重试。
