...
describe('ChatView', () => {
  ...
  it('loads every older history page without retrying a stalled head', async () => {
    ...
    await waitFor(() => { expect(h.loadOlder).toHaveBeenCalledTimes(1) })

    // A settlement whose head did not advance stays available for a manual
    // retry but must not start an automatic tight loop.
    ...
    // key so the freshly opened stream can continue paging.
    act(() => { h.setSession({ openState: 'loading' }) })
    act(() => { h.setSession({ openState: 'open' }) })
    await waitFor(() => { expect(h.loadOlder).toHaveBeenCalledTimes(3) })

    act(() => { h.setSession({ hasMore: false }) })
