...
- `ctx.systemPrompt.assemble(context?: AssembleContext): Promise<PromptAssembly>` Assemble the prompt for one caller: the global layer merged with `context.scope`'s layer, with tool schemas detached before the transform waterfall. Section text providers may be async and are evaluated for every assembly. After the scope-filtered waterfall, an effective complete section replaces ordinary sections, the protected owner section is restored last, and active runtime-context suppressors are enforced. An optional `context.signal` controls this assembly request. Rejects for multiple complete or protected sections, invalid tool order, or a reserved rest-entry tool name.

### Live events

`system-prompt/assemble` is authoritative for ordinary sections. Complete sections constrain that ordinary set, while the single protected owner section is the final DSH-owned authority and is restored after the waterfall. Listeners that replace entries must preserve any active Code Mode or structured-output protocol. Use [`ToolRuntime.restrict()`](../tools/README.md) when filtering must stay aligned across presentation, lookup, and execution. Registry-change notifications are unfiltered.
