...
export function GitView(props: {
  ...
  const openWorktreeDiff = (entry: GitStatusEntry, staged: boolean): void => {
    onOpenDiff({
      ...
      diff: { kind: 'worktree', path: entry.path, staged, untracked: isUntracked(entry), ...(selectedWorktree !== undefined ? { worktree: selectedWorktree } : {}), ...(repoRoot !== undefined ? { repoRoot } : {}) },
    })
  }
  ...
  const openCommitDiff = (entry: GitLogEntry): void => {
    onOpenDiff({
      ...
      diff: { kind: 'commit', hash: entry.hash, hashFull: entry.hashFull, subject: entry.subject, ...(selectedWorktree !== undefined ? { worktree: selectedWorktree } : {}), ...(repoRoot !== undefined ? { repoRoot } : {}) },
    })
  }
      ...
      {status !== null && status.isRepo && (
        <>
          ...
          <div className={css.gitCommit}>
            <Input
              className={clsx(css.gitCommitInput)}
