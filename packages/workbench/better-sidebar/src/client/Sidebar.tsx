...
const TabContent = memo(function TabContent(props: TabContentProps) {
  // oxlint-disable-next-line @stylistic/max-len -- mechanical long line (destructuring / regex / dependency listing).
...
export function Sidebar(props: { ctx: Context; store: SidebarStore }) {
  ...
  const summaryCwd = sessionId === undefined ? undefined : sessionList.byId[sessionId as SessionId]?.cwd
    ...
    const connect = (): void => {
      ...
      socket.onmessage = (event) => {
        ...
        try {
          ...
          if (request.kind === 'url') {
            ctx.get('betterSidebar')?.openTab({ type: 'browser', url: request.target, ...(title !== undefined ? { title } : {}) }, scope)
  ...
  const actions: WorkbenchActions = useMemo(() => ({
    closeTab: (_paneId, tabId) => {
      ...
      ctx.get('betterSidebar')?.closeTab(tabId, sessionId === undefined ? undefined : { sessionId, ...(cwd !== undefined ? { cwd } : {}) })
      if (tab?.type === 'terminal') {
        if (isAgentTabId(tabId)) {
          ...
          // oxlint-disable-next-line @stylistic/max-len -- mechanical long line (destructuring / regex / dependency listing).
          void api.ptyClose({ sessionId, ...(cwd !== undefined ? { cwd } : {}) }, tabId).catch(() => { /* the host may already have released it */ })
        }
      }
    },
    activateTab: (_paneId, tabId) => {
      ...
      ctx.get('betterSidebar')?.activateTab(tabId, sessionId === undefined ? undefined : { sessionId, ...(cwd !== undefined ? { cwd } : {}) })
    },
    ...
    pinTab: (tabId, scope) => {
      store.reduce(s => setTabPin(s, tabId, scope === null ? null : { scope, ...(cwd !== undefined ? { homeCwd: cwd } : {}) }))
    },
  }), [store, sessionId, cwd])
  ...
  const onNewTab = (optionId: string): void => {
    ...
    service.openTab({ type: optionId, title }, { sessionId, ...(cwd !== undefined ? { cwd } : {}) })
  }
  ...
  const tabBadgeOf = (tab: SidebarTab): ReactNode => {
    ...
    try {
      value = descriptor.badge(ctx, { sessionId, ...(cwd !== undefined ? { cwd } : {}) }, state)
    } catch (error) {
      ...
      >
        ...
        <div className={css.panelBody}>
          <Workbench
            state={state}
            tree={augmentedTree}
            newTabOptions={buildNewTabOptions(state, ctx, { sessionId, ...(cwd !== undefined ? { cwd } : {}) })}
      ...
      */}
      {state.floats.map(float => (
        <FreeWindow
          ...
          onClose={() => { ctx.get('betterSidebar')?.closeTab(float.tab.id, sessionId === undefined ? undefined : { sessionId, ...(cwd !== undefined ? { cwd } : {}) }) }}
        />
      ))}
      {/*
