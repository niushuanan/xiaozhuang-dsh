...
export function registerTools(
  ...
  readShellOverrides: () => { shell: string | undefined; shellArgs: string[] | undefined },
): () => void {
  ...
  register(defineTool({
    ...
    execute: async (args: { title: string; command: string }, exec) => {
      ...
      const uuid = await registry.create(sessionId, args.title, args.command, cwd, 80, 24, shell, shellArgs)
