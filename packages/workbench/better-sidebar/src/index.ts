 ...
 * processes are keyed by session. Terminals run through the host subprocess
 * terminal seam (`ctx.subprocess.spawnTerminal`) instead of node-pty.
 */
...
import { defaultShell, PtyManager, shellDisplayName } from './pty-manager.ts'
...
// Re-export the shared Context (the vendored cordis Context intersected with
// the structural service faces) so consumers can type route/service arguments.
 ...
 * store, the web runtime's trusted hosts, the tool registry, and the
 * subprocess terminal seam.
 */
export const inject = ['webServer', 'sessions', 'webRuntime', 'tools', 'subprocess']
...
function shellOverridesOf(
  ...
  if (value === null || typeof value !== 'object') return { shell: undefined, shellArgs: undefined }
 ...
 * @param ctx - host plugin context (webServer, sessions, webRuntime, tools, subprocess).
...
export function apply(ctx: Context, config?: SidebarConfig): void {
  ...
  // The host subprocess terminal seam is a declared injection, so the bound
  // method is stable for the lifetime of the service.
  const spawnTerminal = ctx.subprocess.spawnTerminal.bind(ctx.subprocess)
  // The UI-tab terminal registry, keyed by `${sessionId}:${tabId}` and capped
  // per session; torn down with the plugin.
  const ptyManager = new PtyManager(
    terminalShell,
    resolved.terminalsPerSession,
    resolved.shellArgs,
    resolved.reconnectGraceMs,
    spawnTerminal,
  )
...
/**
 * Pump one live subprocess terminal to a connected view: replay the retained
 * transcript, then follow live output and forward the exit fact once. Returns
 * a disposer that detaches the output listener.
 */
function pumpTerminal(
  ...
  if (transcript !== '') ws.send(transcript)
...
async function attachTerminal(
  ...
  try {
    ...
    const handle = await ptyManager.open(sessionId, tabId, cwd, 80, 24, overrides.shell, overrides.shellArgs)
    const detach = pumpTerminal(handle.handle, handle.transcript, handle.exited, ws)
    ws.on('message', (data) => {
      ...
      if (
        ...
        handle.handle.resize?.(dims.cols, dims.rows)
      } else {
        void handle.handle.write(text).catch(() => { /* teardown race */ })
      }
    })
    ws.on('close', () => {
      detach()
