...
export function isTrustedApiRequest(request: ApiTrustRequest, trustedHosts: readonly string[]): boolean {
  ...
  // Origin fence: when a browser attaches an Origin it must name this exact
  // authority (hostname:port), compared through the same WHATWG normalization
  // as the Host above. Absent Origin is fine — the Host fence already bound
  // the request. The literal "null" (sandboxed iframes, file: pages) is an
  // opaque origin, refused.
