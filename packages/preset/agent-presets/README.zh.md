...
只要 Agent 空闲，会话就能切换到不同的 preset。历史消息与工具调用保持不变；新组装支配后续轮次。Host 会为重链保留一个空闲维护边界，因此活跃轮次不会在执行中途改变，新唤醒输入也会等切换提交后再开始。已提交的切换会发出 `tools/change`，因为解析后的工具集在没有注册表编辑的情况下发生了变化。切换也会记入会话日志，因此恢复或 fork 的会话会按后续轮次所选的组装重建。
...
在每段组装区间内保持前缀稳定。preset 在首次请求前装入；实时切换只发生在轮次之间，下一轮获得新前缀，不应预期跨过该边界复用缓存。其他会话与当前正在执行的请求不受影响。
...
- **活跃轮次中不能更换 preset**——切换会把空闲会话的父作用域重链到另一个常驻挂载。历史消息保持不变；所选组装支配后续轮次。更改部署默认值仍只影响此后创建的会话。
