...
The restriction is now temporal rather than historical: an ACTIVE turn cannot change composition after its request environment was assembled, but a completed conversation can change before the next turn. The gateway owns that boundary ([`dsh-apiproxy`](../../host/apiproxy/README.md)): it uses the Agent maintenance phase to serialize the re-link against new waking input and answers `agent-preset-locked` while another activity owns the boundary. The client may retain that pick and retry once the Agent is idle.
...
- **A preset cannot change during an active turn** — `recompose` re-links an IDLE session's parent scope to another standing mount. Historical messages remain unchanged; the selected composition governs later turns. Changing the deployment default still affects only sessions created afterwards.
